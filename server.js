...
// Optional: Postgres-specific associations (for LLM gateway tables)
try {
  const setupPostgresAssociations = require('./src/models/postgres/associations');
  setupPostgresAssociations();
} catch (e) {
  // No-op: Postgres models may not be present in some deployments.
}
...
require('./src/cronjobs/familyDunningProcessor');
...
// optional: connect to Postgres as well (uses env vars; won't crash if env is missing)
// const { postgresConnection } = require('./src/connection/postgres-connection');
// postgresConnection();
