...
-- 15. Add refund tracking fields to family_payment_transactions
ALTER TABLE `family_payment_transactions`
ADD COLUMN `refund_amount` DECIMAL(10,2) NULL AFTER `amount`,
ADD COLUMN `refund_type` ENUM('full','partial') NULL AFTER `refund_amount`,
ADD COLUMN `refund_reason` VARCHAR(255) NULL AFTER `refund_type`,
ADD COLUMN `refund_date` DATETIME NULL AFTER `refund_reason`,
ADD COLUMN `refund_processed_by` INT(11) UNSIGNED NULL AFTER `refund_date`,
ADD COLUMN `refund_processed_by_name` VARCHAR(255) NULL AFTER `refund_processed_by`,
ADD COLUMN `email_notification_sent` TINYINT(1) NOT NULL DEFAULT 0 AFTER `refund_processed_by_name`;

-- 16. family_children: Default vs Custom pricing (post-payment + edit child)
ALTER TABLE `family_children`
ADD COLUMN `pricing_type` ENUM('default','custom') NULL DEFAULT NULL
COMMENT 'default = standard tier amount; custom = negotiated amount (matches cart planType)'
AFTER `custom_amount`;

-- 17. family_cart_items: persist cart Default / Custom selection
ALTER TABLE `family_cart_items`
ADD COLUMN `cart_pricing_type` ENUM('default','custom') NULL DEFAULT NULL
COMMENT 'Default Plans vs Custom Plan from Family Cart global settings'
AFTER `cart_custom_amount`;