...
-- GAME SESSIONS - ADD COLUMN (link to Postgres analysis row)
ALTER TABLE `game_sessions`
ADD COLUMN `analysis_id` INT NULL;

-- Optional index to speed up status checks
CREATE INDEX `idx_game_sessions_analysis_id` ON `game_sessions` (`analysis_id`);

-- Ensure game_sessions.game_type supports current API game types
ALTER TABLE `game_sessions`
...
-- Ensure mode can store by_class (if mode was previously ENUM)
ALTER TABLE `game_sessions`
MODIFY COLUMN `mode` VARCHAR(50) NULL;
