...
if (!admin.apps.length) {
  ...
  const serviceAccount = path.join('tulkka-firebase-adminsdk-douvv-4b2c75eda1.json');;
  admin.initializeApp({
...
async function whatsappReminderAddClass(templateName, userInfo, userId) {
    try {
        ...
        // Parse notification options (tolerant)
        let notificationOptions = [];
        try {
            notificationOptions = JSON.parse(user.notification_channels || '[]');
        } catch (parseError) {
            logToFile(`Invalid notification_channels JSON for ${user.full_name} (ID: ${userId})`, 'warn', 'class-reminders', {
                userId,
                userName: user.full_name,
                rawValue: user.notification_channels,
        ...
        // Send in-app notification if enabled OR if no other channel succeeded
        const shouldForceInApp = !messageSent;
        if (inAppEnabled || user.isAdmin || shouldForceInApp) {
            ...
            try {
                const inAppSent = await inAppService.sendInAppNotification(
                    templateName,
                    userInfo,
                    userId,
                    user,
                    { force: shouldForceInApp }
                );
