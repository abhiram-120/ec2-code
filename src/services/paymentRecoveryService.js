...
const addRecurringCharge = async (params) => {
    try {
        ...
        const payPlusRequest = {
            ...
            send_failure_callback: true,
            refURL_callback: callback_url || `${process.env.API_BASE_URL}/api/sales/payment-callback/payplus-webhook`
        };
...
/**
 * FAMILY-SPECIFIC: Update recurring payment for a family past-due
 * Currently proxies to updateRecurringPayment but kept separate so
 * family logic can diverge from individual flow in future.
 */
const updateFamilyRecurringPayment = async (params) => {
    return updateRecurringPayment({
        ...params,
        callback_url: `${process.env.API_BASE_URL}/api/sales/family-payment-callback/payplus-webhook`
    });
};

/**
 * FAMILY-SPECIFIC: Add immediate recurring charge for a family past-due
 * Currently proxies to addRecurringCharge but kept separate so
 * family logic can diverge from individual flow in future.
 */
const addFamilyRecurringCharge = async (params) => {
...
const updateRecurringPayment = async (params) => {
    try {
        ...
        const payPlusRequest = {
            ...
            ref_url_callback: callback_url || `${process.env.API_BASE_URL}/api/sales/payment-callback/payplus-webhook`
        };
