...
function buildLevelFromThirdParty(levelText) {
    const label = String(levelText || '').trim();
...
function buildGamesAvailableFromAnalysis(analysis, classId, options = {}) {
    if (!analysis || analysis.game_content_status !== 'ok' || !analysis.game_raw_response) {
        return [];
    }
    ...
    try {
        ...
        console.error(`Failed to parse game_raw_response for class ${classId}:`, error.message);
        return [];
    }
    ...
    return Object.entries(exerciseTypeMap)
        .map(([exercise_type, contentKey]) => {
            const items = Array.isArray(gameContent?.[contentKey]) ? gameContent[contentKey] : [];
...
async function viewProfileV2(req, res) {
    try {
        ...
        // Recent completed class level (from FastAPI analysis)
        let recent_completed_class_level = null;
        try {
            const recentEndedClass = await Class.findOne({
                where: {
                    student_id: user.id,
                    status: 'ended',
                    is_present: 1,
                    zoom_meeting_id: { [Op.ne]: null },
                    meeting_start: { [Op.ne]: null },
                    meeting_end: { [Op.ne]: null },
                },
            ...
            if (recentEndedClass?.zoom_meeting_id && recentEndedClass?.meeting_start && recentEndedClass?.meeting_end) {
                const statusResult = await getClassAnalysisStatus(
                    recentEndedClass.zoom_meeting_id,
                    new Date(recentEndedClass.meeting_start).toISOString(),
                    new Date(recentEndedClass.meeting_end).toISOString()
                );

                if (statusResult?.found && statusResult?.status === 'completed' && statusResult?.request_id) {
                    const analysis = await getAnalysisResults(statusResult.request_id);
                    if (analysis?.game_content_status === 'ok' && analysis?.level) {
                        recent_completed_class_level = buildLevelFromThirdParty(analysis.level);
                    }
                }
            }
        } catch (e) {
            // Don't fail profile response if AI service is unavailable
            console.error('viewProfileV2 recent_completed_class_level error:', e.message);
        }
...
async function homeScreenDetails(req, res) {
  ...
  try {
    ...
    // 🛡 SAFE EXECUTOR
    // =========================
    const safeExecute = async (fn, fallback, message) => {
      try {
        return await fn();
      } catch (err) {
    ...
    // 📅 UPCOMING CLASS
    // =========================
    const upcomingClass = await safeExecute(
      () =>
        Class.findOne({
          where: {
            student_id: userId,
          ...
          attributes: ['id', 'teacher_id', 'meeting_start', 'meeting_end']
        }),
      null,
      'Failed to fetch upcoming class'
    );

    if (!upcomingClass) {
      fallbackMessages.push('No upcoming class found for this student');
    }
    ...
    // =========================
    const teacher =
      upcomingClass?.teacher_id
        ? await safeExecute(
            () => Users.findByPk(upcomingClass.teacher_id),
            null,
    ...
    if (teacher) {
      try {
        [occupations, totalReviewer, rates] = await Promise.all([
        ...
        console.error('Failed to fetch teacher related data', err);
        fallbackMessages.push('Unable to fetch teacher rating data');
        occupations = [];
        totalReviewer = 0;
        rates = [];
      }
    ...
    const avgRate =
      totalReviewer > 0
        ? rates.reduce((sum, r) => {
            const value = parseFloat(r?.rates);
            return sum + (isNaN(value) ? 0 : value);
          }, 0) / totalReviewer
        : 0;

    // =========================
    // 📊 LATEST ANALYSIS (Postgres)
    // =========================
    let analysis = null;
    ...
    // 💳 ACTIVE SUBSCRIPTION
    // =========================
    const activeSubscription = await safeExecute(
      () =>
        UserSubscriptionDetails.findOne({
          where: {
            user_id: userId,
    ...
    if (activeSubscription) {
      ...
      try {
        const [total, ended, pending] = await Promise.all([
          Class.count({
            where: {
              ...baseWhere,
          ...
          Class.count({
            where: {
              ...baseWhere,
              status: 'pending',
              meeting_start: { [Op.gte]: now }
            }
          })
        ]);

        classCounts = {
          total_classes: total || 0,
          ended_classes: ended || 0,
          pending_classes: pending || 0
        };
      } catch (err) {
        console.error('Failed to fetch class counts', err);
        fallbackMessages.push('Unable to fetch class counts');
      }
    }

    let lastClassEndedDate = null;
    if (
      activeSubscription &&
      classCounts.pending_classes > 0 &&
      classCounts.ended_classes > 0 &&
      ((Number(activeSubscription.left_lessons) || 0) > 0 ||
        (Number(activeSubscription.weekly_lesson) || 0) > 0)
    ) {
      const lastEndedRow = await safeExecute(
        () =>
          Class.findOne({
            where: {
              subscription_id: activeSubscription.id,
        ...
        null,
        'Failed to fetch last class ended date'
      );
      if (lastEndedRow?.meeting_end) {
        lastClassEndedDate = lastEndedRow.meeting_end;
      }
    }
    ...
    // =========================
    const userTimezone =
      moment.tz.zone(user?.timezone) ? user.timezone : 'UTC';

    const weeklyEnd = moment.tz(userTimezone).endOf('day');
    const weeklyStart = weeklyEnd.clone().subtract(6, 'days').startOf('day');

    const weeklyStartUtc = weeklyStart.clone().tz('UTC').toDate();
    const weeklyEndUtc = weeklyEnd.clone().tz('UTC').toDate();

    // =========================
    // 🎮 RECENT ENDED CLASS + AI ANALYSIS
    // =========================
    const recentEndedClass = await safeExecute(
      () =>
        Class.findOne({
          where: {
            student_id: userId,
      ...
      null,
      'Failed to fetch recent ended class'
    );
    ...
    if (recentEndedClass?.zoom_meeting_id && recentEndedClass?.meeting_start && recentEndedClass?.meeting_end) {
      const statusResult = await safeExecute(
        ...
        null,
        'Failed to fetch class analysis status from FastAPI'
      );

      if (statusResult?.found && statusResult?.status === 'completed' && statusResult?.request_id) {
        aiRequestId = statusResult.request_id;

        aiAnalysis = await safeExecute(
          () => getAnalysisResults(statusResult.request_id),
          null,
          'Failed to fetch analysis results from FastAPI'
        );

        if (aiAnalysis?.game_content_status === 'ok') {
          const GAME_TYPES = ['flashcard', 'fill_blank', 'spelling_bee', 'grammar_challenge', 'sentence_builder', 'advanced_cloze'];
          gamesAvailable = GAME_TYPES.map((exercise_type) => ({
            exercise_type,
            class_id: recentEndedClass.id,
            meeting_id: recentEndedClass.zoom_meeting_id,
            meeting_start: recentEndedClass.meeting_start,
            meeting_end: recentEndedClass.meeting_end
          }));
        }
      }
    }

    // Build analysis response from FastAPI result
    let analysisResponse = null;
    if (aiAnalysis) {
      let teacherPlannedNextStep = null;
      try {
        const rawText = aiAnalysis.raw_response;
        if (rawText) {
          const parsed = JSON.parse(rawText);
          const analysisData = parsed.response_text ? JSON.parse(parsed.response_text) : parsed;
          teacherPlannedNextStep = analysisData?.teacher_performance?.teacher_planned_next_step || null;
        }
      } catch (e) {
        console.error('Failed to parse raw_response for teacher_planned_next_step:', e.message);
      }

      analysisResponse = {
        id: aiAnalysis.id,
        request_id: aiAnalysis.request_id,
        zoom_meeting_id: aiAnalysis.zoom_meeting_id,
        level: aiAnalysis.level,
        summary: aiAnalysis.summary,
        general_comment: aiAnalysis.general_comment,
        general_comment_version: aiAnalysis.general_comment_version,
        grammar_score: aiAnalysis.grammar_score,
        vocabulary_score: aiAnalysis.vocabulary_score,
        fluency_score: aiAnalysis.fluency_score,
        score_method: aiAnalysis.score_method,
        engagement_level: aiAnalysis.engagement_level,
        lesson_headline: aiAnalysis.lesson_headline,
        practice_targets: aiAnalysis.practice_targets,
        improvement_signal: aiAnalysis.improvement_signal,
        progress_badge: aiAnalysis.progress_badge,
        progress_badge_reason: aiAnalysis.progress_badge_reason,
        parent_summary: aiAnalysis.parent_summary,
        next_lesson_focus: aiAnalysis.next_lesson_focus,
        correction_approach: aiAnalysis.correction_approach,
        vocabulary_words: aiAnalysis.vocabulary_words,
        grammar_points: aiAnalysis.grammar_points,
        grammar_feedback: aiAnalysis.grammar_feedback,
        vocabulary_feedback: aiAnalysis.vocabulary_feedback,
        pronunciation_feedback: aiAnalysis.pronunciation_feedback,
        game_content_status: aiAnalysis.game_content_status,
        created_at: aiAnalysis.created_at,
        student_performance: {
          scores: {
            grammar: aiAnalysis.grammar_score,
            vocabulary: aiAnalysis.vocabulary_score,
            fluency: aiAnalysis.fluency_score
          }
        },
        raw_analysis: {
          teacher_performance: {
            ai_suggested_syllabus: aiAnalysis.next_lesson_focus?.suggested_activity || null,
            teacher_planned_next_step: teacherPlannedNextStep
          }
        }
      };
    }
    ...
    // 📅 WEEKLY HIGHLIGHTS
    // =========================
    const weeklyResults = await safeExecute(
      () =>
        Promise.all([
          GameSession.count({
            where: {
              ...
              status: 'completed',
              [Op.or]: [
                { completed_at: { [Op.between]: [weeklyStartUtc, weeklyEndUtc] } },
                {
                  completed_at: null,
                  updated_at: { [Op.between]: [weeklyStartUtc, weeklyEndUtc] }
                }
              ]
            }
          }),
          Class.findAll({
            ...
            where: {
              student_id: userId,
              status: 'ended',
              meeting_start: { [Op.between]: [weeklyStartUtc, weeklyEndUtc] }
            }
          }),
          AudioBroadcast.findOne({
            attributes: [[Sequelize.fn('SUM', Sequelize.col('listens')), 'total_listens']],
            where: {
              updated_at: { [Op.between]: [weeklyStartUtc, weeklyEndUtc] }
            },
            raw: true
          }),
          Promise.resolve(analysis ? { vocabulary_words: analysis.vocabulary_words } : null)
        ]),
      [0, [], null, null],
      'Failed to fetch weekly highlights'
    );

    const [
      gamePlayedCount = 0,
      endedClassesRaw = [],
      podcastListenRow = null,
      vocabularyWordsRow = null
    ] = weeklyResults;

    const endedClasses = Array.isArray(endedClassesRaw)
      ? endedClassesRaw
      : [];

    const totalLearningMs = endedClasses.reduce((sum, cls) => {
      if (!cls?.meeting_start || !cls?.meeting_end) return sum;
      const diff =
    ...
    const learningHours =
      Math.round((totalLearningMs / 3600000) * 10) / 10;

    const podcastsHeard =
      parseInt(podcastListenRow?.total_listens, 10) || 0;

    let vocabularyWords = vocabularyWordsRow?.vocabulary_words;

    if (typeof vocabularyWords === 'string') {
      try {
        vocabularyWords = JSON.parse(vocabularyWords);
      } catch {
        vocabularyWords = [];
      }
    }

    const vocabularyWordsCount = Array.isArray(vocabularyWords)
      ? vocabularyWords.length
      : 0;

    const weekly_highlights = [
      { key: 'game_played', title: 'Game Played', value: gamePlayedCount },
      { key: 'podcasts_heard', title: 'Podcasts Heard', value: podcastsHeard },
      { key: 'learning', title: 'Learning', value: learningHours, unit: 'h' },
      { key: 'vocabulary_words', title: 'Vocabulary Words', value: vocabularyWordsCount }
    ];

    const unread_notifications_count = await safeExecute(
      () => firebaseService.getUnreadCount(userId),
      0,
      'Failed to fetch unread notifications count'
    );

    const leftLessonsNum = activeSubscription
      ? Number(activeSubscription.left_lessons) || 0
      : 0;
    const bonusClassNum = activeSubscription
      ? Number(activeSubscription.bonus_class) || 0
      : 0;
    const count = activeSubscription
      ? {
          this_month: activeSubscription.weekly_lesson ?? null,
          carried_over: leftLessonsNum - bonusClassNum,
          bonus: bonusClassNum
        }
      : null;
      ...
      data: {
        ...
        teacher: teacher
          ? {
              ...
              occupations: {
                specialties: occupations.filter(o => o?.type === 'specialties').map(o => o.value),
                also_speaking: occupations.filter(o => o?.type === 'also_speaking').map(o => o.value),
                teachings: occupations.filter(o => o?.type === 'teachings').map(o => o.value),
                levels: occupations.filter(o => o?.type === 'levels').map(o => o.value)
              },
              rate: {
        ...
        subscription: activeSubscription
          ? {
              ...
              left_lessons: activeSubscription.left_lessons,
              class_counts: classCounts,
              last_class_ended_date: lastClassEndedDate
            }
          : null,
    ...
    console.error('homeScreenDetails fatal error:', error);
    return res.status(500).json({
      status: 'error',
      message: 'Internal server error'
    });
...
async function editClassV2(req, res) {
    try {
        ...
        try {
            ...
            // Notify student about the reschedule (in-app supported template)
            const notifyOptionsStudent = {
                teacherName: teacherData2.full_name,
                newDateTime: moment(req.body.meeting_start).tz(user.timezone).format('DD/MM/YYYY HH:mm')
            };
            await whatsappReminderAddClass('class_rescheduled_success', notifyOptionsStudent, user.id);
        } catch (error) {
            console.error('Error calling whatsappReminderAddClass:', error);
            // Handle error appropriately
        }
...
async function viewClasses(req, res) {
    try {
        ...
        const [teachers, allHomework, allStudentAnswers, allClassQueries, cancelledByUsers] = await Promise.all([
            Users.findAll({
                attributes: ['id', 'full_name', 'about', 'language', 'avatar', 'video_demo', 'video_demo_thumb', 'enable_zoom_link', 'add_zoom_link', 'add_zoom_link_meeting_id', 'add_zoom_link_access_code'],
                where: { id: teacherIds }
            }),
        ...
        const cancelledByUserMap = cancelledByUsers.reduce((acc, userItem) => {
            acc[userItem.id] = userItem;
            return acc;
        }, {});

        // Games logic same as homeScreenDetails: use most recent ended class analysis
        let gamesAvailableForUser = [];
        const recentEndedClass = await Class.findOne({
            where: {
                student_id: userId,
                status: 'ended'
        ...
        if (recentEndedClass && recentEndedClass.zoom_meeting_id && recentEndedClass.meeting_start && recentEndedClass.meeting_end) {
            try {
                const statusResult = await getClassAnalysisStatus(
                    recentEndedClass.zoom_meeting_id,
                    new Date(recentEndedClass.meeting_start).toISOString(),
                ...
                if (statusResult?.found && statusResult?.status === 'completed' && statusResult?.request_id) {
                    const analysisResult = await getAnalysisResults(statusResult.request_id);
                    gamesAvailableForUser = buildGamesAvailableFromAnalysis(
                        analysisResult,
                        recentEndedClass.id
                    );
                }
            } catch (err) {
                gamesAvailableForUser = [];
            }
        }

        // Feedback logic same as aiFeedback: fetch ended classes -> status -> completed analyses
        const analysisByTeacherId = {};
        const teachersWithAiFeedback = new Set();
        const endedClassesForFeedback = await Class.findAll({
            where: {
                student_id: userId,
                status: 'ended',
                teacher_id: { [Op.in]: [...new Set(teacherIds)] }
            },
        ...
        const zoomEndedClasses = endedClassesForFeedback.filter(
            (cls) => cls.zoom_meeting_id && cls.meeting_start && cls.meeting_end
        );

        if (zoomEndedClasses.length > 0) {
            const statusResults = await Promise.all(
                zoomEndedClasses.map((cls) =>
                    getClassAnalysisStatus(
                        cls.zoom_meeting_id,
                ...
                const analysisResults = await Promise.all(
                    completedItems.map(({ cls, status }) =>
                        getAnalysisResults(status.request_id)
                            .then((analysis) => ({ cls, analysis }))
                            .catch(() => null)
                    )
                );

                analysisResults.filter(Boolean).forEach(({ cls, analysis }) => {
                    if (!teachersWithAiFeedback.has(cls.teacher_id)) {
                        analysisByTeacherId[cls.teacher_id] = analysis;
                        teachersWithAiFeedback.add(cls.teacher_id);
                    }
                });
            }
        }
        ...
        const response = classData.map((classItem) => {
            ...
            return {
                ...
                cancellation_reason: classItem.cancellation_reason || null,
                cancelled_by: classItem.cancelled_by || null,
                cancelled_at: classItem.cancelled_at
                    ? getLocalDateTime(classItem.cancelled_at, timezone)
                    : null,
                canceled_by: classItem.canceled_by || null,
                cancelled_by_user: classItem.cancelled_by
                    ? {
                        id: classItem.cancelled_by,
                        full_name: cancelledByUserMap[classItem.cancelled_by]?.full_name || null,
                        role_name: cancelledByUserMap[classItem.cancelled_by]?.role_name || null
                    }
                    : null,
    ...
    try {
        ...
        const classItem = await Class.findOne({
            attributes: [
                ...
                'status', 'join_url', 'admin_url', 'feedback_id', 'zoom_meeting_id',
        ...
        if (classItem.is_regular_hide === true || classItem.is_regular_hide === 1) {
            return res.status(404).json({ status: 'error', message: 'Class not found or you do not have access to this class' });
        }
        ...
        // 4. Fetch homework, answers, and queries in parallel
        const [homework, studentAnswers, classQueries] = await Promise.all([
            Homework.findOne({
                attributes: [
                    'id',
                    'description',
        ...
        // Same as filterClassV2: user-level AI feedback (not teacher-based)
        let latestUserAnalysis = null;
        let hasAnyAiFeedback = false;
        const endedClassesForFeedback = await Class.findAll({
            where: { student_id: userId, status: 'ended' },
            attributes: ['zoom_meeting_id', 'meeting_start', 'meeting_end'],
            order: [['meeting_start', 'DESC']],
            raw: true
        });
        const zoomEndedClasses = endedClassesForFeedback.filter(
        ...
        if (zoomEndedClasses.length > 0) {
            ...
            const recentCompleted = statusResults.find(
                (item) => item.status?.found && item.status?.status === 'completed' && item.status?.request_id
            );
            if (recentCompleted) {
                try {
                    latestUserAnalysis = await getAnalysisResults(recentCompleted.status.request_id);
                    hasAnyAiFeedback = Boolean(latestUserAnalysis);
                } catch (err) {
                    latestUserAnalysis = null;
                    hasAnyAiFeedback = false;
                }
            }
        }

        // Games for this class (fallback to most recent ended class if unavailable)
        let gamesAvailableForUser = [];
        ...
        // fallback (same as home-screen/filter): latest ended class games
        if (!gamesAvailableForUser || (Array.isArray(gamesAvailableForUser) && gamesAvailableForUser.length === 0)) {
            const recentEndedClass = await Class.findOne({
                where: { student_id: userId, status: 'ended' },
                order: [['meeting_start', 'DESC']],
                attributes: ['id', 'zoom_meeting_id', 'meeting_start', 'meeting_end']
            ...
            if (recentEndedClass && recentEndedClass.zoom_meeting_id && recentEndedClass.meeting_start && recentEndedClass.meeting_end) {
                try {
                    ...
                    console.error('Error in viewClassDetails to get the games data :', err);
                    gamesAvailableForUser = [];
                }
            }
        }

        let wordLearned = latestUserAnalysis?.vocabulary_words || [];
        if (typeof wordLearned === 'string') {
            try {
                wordLearned = JSON.parse(wordLearned);
            } catch (err) {
                wordLearned = [];
            }
        }
        ...
        const response = {
            ...
            teacher_feedback: {
                strengths: latestUserAnalysis?.grammar_feedback || null,
                areas_for_improvement: latestUserAnalysis?.pronunciation_feedback || null,
            },
            teacher_feedback_v2: {
                grammar_feedback: latestUserAnalysis?.grammar_feedback || null,
                pronunciation_feedback: latestUserAnalysis?.pronunciation_feedback || null,
                vocabulary_feedback: latestUserAnalysis?.vocabulary_feedback || null
            },

            ai_feedback: hasAnyAiFeedback,
            general_comment: latestUserAnalysis?.general_comment || null,
            ...
            lesson_summary: latestUserAnalysis?.summary || null,
            word_learned: Array.isArray(wordLearned) ? wordLearned : [],
            class_duration: classDuration,
            recording_status: classItem.recording_status,
            recording_url: classItem.recording_url,
            games_available: gamesAvailableForUser,
        ...
        console.error('Error in viewClassDetails:', {
            message: error?.message,
            stack: error?.stack,
            userId: req?.userId,
...
async function cancelClassV2(req, res) {
    try {
        ...
        try {
            const notifyOptionsTeacher = {
                'instructor.name': teacherData2?.full_name || null,
                'student.name': user.full_name,
                'class.time': moment(classes.meeting_start).tz(teacherData2.timezone).format('DD/MM/YYYY HH:mm')
                // link: classes_url
            };
            const notifyOptionsStudent = {
                teacherName: teacherData2?.full_name || null
            };
            await whatsappReminderAddClass('student_class_cancelled', notifyOptionsTeacher, classes.teacher_id);
            await whatsappReminderAddClass(
                teacherData2?.full_name ? 'class_cancelled_success' : 'class_cancelled_success_no_teacher',
...
async function deleteHomework(req, res) {
    try {
        ...
        // Best-effort lookup for notifications (do not affect delete logic)
        let homeworkMeta = null;
        try {
            const hw = await Homework.findOne({ where: { id: homeworkId } });
            if (hw) {
                homeworkMeta = {
                    studentId: hw.student_id || null,
                    teacherId: hw.teacher_id || null,
                    homeworkTitle: hw.title || hw.homework_title || hw.description || null,
                };
            }
        } catch (metaErr) {
            // ignore
        ...
        if (!deletedHomework || deletedHomework.length === 0) {
            // Best-effort in-app notification for failure (do not affect response)
            try {
                await whatsappReminderAddClass('homework_deletion_failed', {}, user.id);
            } catch (notifyErr) {
                // ignore
            }
        ...
        // Best-effort notifications to student + teacher (do not affect response)
        try {
            const template = homeworkMeta?.homeworkTitle ? 'homework_deleted_success' : 'homework_deleted_success_no_title';
            const notifyOptions = homeworkMeta?.homeworkTitle ? { homeworkTitle: String(homeworkMeta.homeworkTitle) } : {};

            if (homeworkMeta?.studentId) {
                await whatsappReminderAddClass(template, notifyOptions, homeworkMeta.studentId);
            }
            if (homeworkMeta?.teacherId) {
                await whatsappReminderAddClass(template, notifyOptions, homeworkMeta.teacherId);
            }
        } catch (notifyErr) {
            // ignore
        }
...
async function submitHomework(req, res) {
    try {
        ...
        // Notify student: homework submitted successfully (in-app supported)
        try {
            const homeworkTitle =
                submitHomework?.title ||
                submitHomework?.homework_title ||
                submitHomework?.description ||
                null;

            const template = homeworkTitle
                ? 'homework_submitted_success'
                : 'homework_submitted_success_no_title';

            const notifyOptions = homeworkTitle ? { homeworkTitle: String(homeworkTitle) } : {};

            await whatsappReminderAddClass(template, notifyOptions, user.id);
        } catch (error) {
...
// view all AI feedback for particular students
async function aiFeedback(req, res) {
    try {
        ...
        // Get ended classes with Zoom info, filtered by meeting date range if provided
        const classWhere = {
            student_id: user.id,
            status: 'ended',
        };
        if (from_date) {
            classWhere.meeting_start = {
                ...(classWhere.meeting_start || {}),
                [Op.gte]: moment(from_date, 'YYYY-MM-DD').startOf('day').toDate()
            };
        }
        ...
        if (!classes || classes.length === 0) {
            return res.status(200).json({ status: 'error', message: 'No classes found' });
        }

        // Only consider classes that have Zoom recording info
        ...
        // Fetch teacher profiles, occupations, reviews in one batch
        const teacherIds = [...new Set(validResults.map((r) => r.cls.teacher_id).filter(Boolean))];

        const [teachers, occupations, reviews] = await Promise.all([
            teacherIds.length > 0
                ? Users.findAll({
                    where: { id: teacherIds },
                    attributes: [
                        'id', 'full_name', 'about', 'language', 'avatar',
                        'video_demo', 'video_demo_thumb', 'enable_zoom_link',
                        'add_zoom_link', 'add_zoom_link_meeting_id', 'add_zoom_link_access_code'
                    ],
                    raw: true
                })
                : Promise.resolve([]),
            ...
            teacherIds.length > 0
                ? UserReview.findAll({
                    attributes: ['instructor_id', 'rates'],
                    where: { instructor_id: { [Op.in]: teacherIds } },
                    raw: true
                })
                : Promise.resolve([]),
        ]);
        ...
        const occupationMap = occupations.reduce((map, occ) => {
            if (!map[occ.user_id]) map[occ.user_id] = [];
            map[occ.user_id].push(occ);
            return map;
        }, {});

        const ratingAgg = reviews.reduce((map, r) => {
            if (!map[r.instructor_id]) map[r.instructor_id] = { total: 0, sum: 0 };
            map[r.instructor_id].total += 1;
            map[r.instructor_id].sum += parseFloat(r.rates || 0);
            return map;
        }, {});
        const ratingMap = teacherIds.reduce((map, tid) => {
            const agg = ratingAgg[tid];
            map[tid] = agg
                ? { total_reviews: agg.total, avgRate: agg.total > 0 ? agg.sum / agg.total : 0 }
                : { total_reviews: 0, avgRate: 0 };
            return map;
        }, {});

        // Build formatted feedbacks from FastAPI analysis data
        let formattedFeedbacks = validResults.map(({ cls, analysis }) => {
            const relatedTeacher = cls.teacher_id ? teacherMap[cls.teacher_id] : null;
            const teacherOccupations = relatedTeacher ? (occupationMap[relatedTeacher.id] || []) : [];
            const teacherRating = relatedTeacher ? (ratingMap[relatedTeacher.id] || { total_reviews: 0, avgRate: 0 }) : { total_reviews: 0, avgRate: 0 };

            let classDuration = null;
            if (cls.meeting_start && cls.meeting_end) {
                const diffMs = new Date(cls.meeting_end) - new Date(cls.meeting_start);
                const totalMinutes = Math.floor(diffMs / 60000);
                classDuration = `${Math.floor(totalMinutes / 60)}h ${totalMinutes % 60}m`;
            }

            const currentTime = new Date();
            const meetingStartTime = cls.meeting_start ? new Date(cls.meeting_start) : null;
            const fourHoursInMillis = 4 * 60 * 60 * 1000;
            const isWithinFourHours = meetingStartTime
                ? (meetingStartTime - currentTime) <= fourHoursInMillis && (meetingStartTime - currentTime) > 0
                : false;
            ...
            return {
                ...
                vocabulary_learned: analysis.vocabulary_words,
                grammar_concepts: analysis.grammar_points,
                ...
                class: {
                    ...
                    feedback: Boolean(cls.feedback_id),
                    recording_status: cls.recording_status,
                    recording_url: cls.recording_url,
                    timezone: user.timezone,
                    meeting_start_local: getLocalDateTime(cls.meeting_start, user.timezone),
                    meeting_end_local: getLocalDateTime(cls.meeting_end, user.timezone),
                    is_within_four_hours: isWithinFourHours,
                    class_duration: classDuration
                },
                teacher: relatedTeacher ? {
                    id: relatedTeacher.id,
                    full_name: relatedTeacher.full_name,
                    about: relatedTeacher.about,
                    language: relatedTeacher.language,
                    avatar: relatedTeacher.avatar,
                    video_demo: relatedTeacher.video_demo,
                    video_demo_thumb: relatedTeacher.video_demo_thumb,
                    enable_zoom_link: relatedTeacher.enable_zoom_link,
                    add_zoom_link: relatedTeacher.add_zoom_link,
                    add_zoom_link_meeting_id: relatedTeacher.add_zoom_link_meeting_id,
                    add_zoom_link_access_code: relatedTeacher.add_zoom_link_access_code,
                    occupations: {
                        specialties: teacherOccupations.filter((o) => o.type === 'specialties').map((o) => o.value),
                        also_speaking: teacherOccupations.filter((o) => o.type === 'also_speaking').map((o) => o.value),
                        teachings: teacherOccupations.filter((o) => o.type === 'teachings').map((o) => o.value),
                        levels: teacherOccupations.filter((o) => o.type === 'levels').map((o) => o.value)
                    },
                    rate: {
                        total_reviews: teacherRating.total_reviews,
                        avgRate: teacherRating.avgRate
        ...
        if (teacher_id) {
            formattedFeedbacks = formattedFeedbacks.filter(
                (fb) => fb.teacher && fb.teacher.id.toString() === teacher_id.toString()
            );
        }

        if (sort_by === 'teacher_name') {
            formattedFeedbacks.sort((a, b) => {
                const nameA = a.teacher?.full_name || '';
                const nameB = b.teacher?.full_name || '';
                return validSortOrder === 'ASC' ? nameA.localeCompare(nameB) : nameB.localeCompare(nameA);
            });
        } else {
            formattedFeedbacks.sort((a, b) => {
        ...
        return res.status(500).json({
            status: 'error',
            message: 'An error occurred while fetching AI feedbacks'
        });
    }
}
...
async function filterClassV2(req, res) {
    try {
        ...
        /** Used for status filter + meeting_start sort (must exist when status query is omitted). */
        ...
        if (status) {
            const allowedStatuses = new Set(['ended', 'canceled', 'pending']);

            // Supports:
            // - status=ended
            // - status=ended,canceled
            // - status[]=ended&status[]=pending
            const rawStatusValues = Array.isArray(status)
                ? status
                : String(status).split(',');

            normalizedStatuses = rawStatusValues
            ...
            if (normalizedStatuses.length > 0) {
                filterObject.status = {
                    [Op.in]: [...new Set(normalizedStatuses)]
                };
            }
        }

        const uniqueStatuses = [...new Set(normalizedStatuses)];
        const sortMeetingStartAsc =
        ...
        let data = await Class.findAll({
            ...
            order: [
                ['status', 'DESC'],
                ['meeting_start', sortMeetingStartAsc ? 'ASC' : 'DESC']
            ],
            limit: limit,
            offset: offset
        });
        ...
        const cancelledByIds = [
            ...new Set(
                data
                    .map((classItem) => classItem.cancelled_by)
                    .filter((id) => !!id)
            )
        ];
        ...
        // Fetch occupations, reviews and cancelled-by users in parallel
        const [allOccupations, allReviews, cancelledByUsers] = await Promise.all([
            UserOccupation.findAll({
                attributes: ['user_id', 'type', 'value'],
                where: {
                    user_id: teacherIds
        ...
        // Games logic same as homeScreenDetails (most recent ended class)
        let gamesAvailableForUser = [];
        const recentEndedClass = await Class.findOne({
            where: { student_id: userId, status: 'ended' },
            order: [['meeting_start', 'DESC']],
        ...
        // AI feedback logic (not teacher-based): latest completed analysis for user ended classes
        let latestUserAnalysis = null;
        let hasAnyAiFeedback = false;
        const endedClassesForFeedback = await Class.findAll({
            where: { student_id: userId, status: 'ended' },
        ...
        let response = await Promise.all(
            data.map(async (classItem) => {
                ...
                const classAnalysis = latestUserAnalysis;
                const hasTeacherFeedback = hasAnyAiFeedback;
                let wordLearned = classAnalysis?.vocabulary_words || [];
                if (typeof wordLearned === 'string') {
                    try {
                        wordLearned = JSON.parse(wordLearned);
                    } catch (err) {
                ...
                return {
                    ...
                    ai_feedback: hasTeacherFeedback,
                    timezone: user.timezone,
                    is_trial: classItem.is_trial,
                    meeting_start: localMeetingStartTime,
                    meeting_end: localMeetingEndTime,
                    ...
                    has_queries: classQueries.length > 0,
                    is_within_four_hours: isWithinFourHours,
                    lesson_summary: classAnalysis?.summary || null,
                    word_learned: Array.isArray(wordLearned) ? wordLearned : [],
                    class_duration: classDuration,
                    recording_status: classItem.recording_status,
                    recording_url: classItem.recording_url,
...
async function submitClassQuery(req, res) {
    try {
        ...
        // In-app notification for student when attachments uploaded
        try {
            if (req.files && req.files.length > 0) {
                const firstName =
                    req.files[0]?.originalname ||
                    (req.files[0]?.key ? String(req.files[0].key).split('/').pop() : null) ||
                    null;
                ...
                await whatsappReminderAddClass('file_uploaded_success', { fileName }, user.id);
            }
        } catch (notifyErr) {
            console.error('Error sending file_uploaded_success notification:', notifyErr);
        }
...
async function getTeacherAvailability(req, res) {
    try {
        ...
        teachers.forEach(teacher => {
            ...
            // Convert each teacher's availability to the USER timezone
            const userTimezoneSchedule = convertScheduleToUserTimezone(parsedAvailability, userTimezone);

            teacherDataMap.set(teacher.id, {
                teacher,
                parsedAvailability,
                userTimezoneSchedule,
        ...
        // Current time in the user's timezone; all 8h logic is based on this
        ...
        for (let hour = 0; hour < 24; hour++) {
            for (let minute of [0, 30]) {
                ...
                // Skip past slots based on USER local time
                if (slotStartUser.isSameOrBefore(nowUser)) {
                    continue;
                }

                // 8h window calculated in USER timezone
                const diffInHoursFromNow = slotStartUser.diff(nowUser, 'hours', true);
                const isWithinNext8Hours = diffInHoursFromNow > 0 && diffInHoursFromNow < 8;

                const availableTeachers = [];
                let availableCount = 0;

                teacherDataMap.forEach((data, teacherId) => {
                    const { teacher, userTimezoneSchedule, convertedHolidays, rating } = data;
                    const teacherOccupations = data.occupations || [];
                    const daySchedule = userTimezoneSchedule ? userTimezoneSchedule[dayOfWeek] : null;
                    ...
                    // Enforce 8-hour booking restriction similar to teacherAvailabilityV2
                    if (isWithinNext8Hours) {
                        teacherStatus.message = "Slots are not bookable within the next 8 hours";
                        availableTeachers.push(teacherStatus);
                        return;
                    }
                ...
                const slotStartInUserTz = slotStartUTC.clone().tz(userTimezone);

                timeSlotAvailability[timeKey] = {
                    count: availableCount,
                    teachers: availableTeachers,
                    utc: slotStartUTC.format(),
                    local: slotStartInUserTz.format(),
                    localTime: slotStartInUserTz.format('HH:mm'),
...
/**
 * Public FAQs for user app (no auth).
 * Returns active FAQs with translations + attachment info.
 */
async function getFaqs(req, res) {
    try {
        ...
        return res.status(200).json({
            ...
            data,
            mobile_number:'+97243033128',
            email: 'Info@tulkka.com'
        });
    } catch (error) {
