...
// Normalize phone numbers so DB doesn't store spaces/formatting.
// We keep digits (and an optional leading '+') only.
...
/** Prefer DB pricing_type; legacy rows use heuristics (for Families list Default/Custom). */
const deriveIsCustomPricing = (childData) => {
    if (childData.pricing_type === 'custom') return true;
    if (childData.pricing_type === 'default') return false;
    try {
        let isCustomPricing = false;
        const monthlyAmount = parseFloat(childData.monthly_amount || 0);

        if (childData.current_subscription && monthlyAmount > 0) {
            const costPerLesson = parseFloat(childData.current_subscription.cost_per_lesson || 0);
            const weeklyLesson = parseFloat(childData.current_subscription.weekly_lesson || 0);

            if (costPerLesson > 0 && weeklyLesson > 0) {
                const defaultMonthly = costPerLesson * weeklyLesson * 4;
                if (Math.abs(monthlyAmount - defaultMonthly) > 0.01) {
                    isCustomPricing = true;
                }
            }
        }

        if (!childData.current_subscription) {
            const customAmount = parseFloat(childData.custom_amount || 0);
            if (customAmount > 0) {
                isCustomPricing = true;
            }
        }
...
const getFamilyList = async (req, res) => {
    try {
        ...
        if (search) {
            const searchNoSpace = String(search).replace(/\s+/g, '');
            whereConditions[Op.or] = [
                { parent_name: { [Op.like]: `%${search}%` } },
                { parent_email: { [Op.like]: `%${search}%` } },
                { parent_phone: { [Op.like]: `%${search}%` } },
                // Allow searching phone even if stored without spaces (or vice versa)
                Sequelize.where(
                    fn('REPLACE', col('parent_phone'), ' ', ''),
                    { [Op.like]: `%${searchNoSpace}%` }
                )
            ];
        }
        ...
        const familiesWithSubscriptionData = await Promise.all(families.map(async (family) => {
            ...
            const enhancedChildren = await Promise.all(
                family.children.map(async (child) => {
                    ...
                    childData.is_custom_pricing = deriveIsCustomPricing(childData);
...
const createFamily = async (req, res) => {
    ...
    try {
        ...
        if (!parentName || !parentEmail || !parentPhone || !children || children.length === 0) {
            if (transaction) await transaction.rollback();
            return res.status(400).json({
                status: 'error',
                message: 'Parent name, email, phone, and at least one child are required'
            });
        }
        ...
        // Basic phone validation (optional: allow +, digits, spaces, dashes)
        ...
        // Check if family with this phone already exists
        const existingPhoneFamily = await Family.findOne({
        ...
        if (existingPhoneFamily) {
            if (transaction) await transaction.rollback();
            return res.status(409).json({
                status: 'error',
                message: 'Family with this phone number already exists'
            });
        }
...
const updateFamily = async (req, res) => {
    ...
    try {
        ...
        await family.update({
            ...
            parent_phone: normalizedParentPhone || family.parent_phone,
    ...
    try {
        ...
        await logFamilyActivity({
            ...
            metadata: {
                ...
                // Shown in Admin → Family → Transactions "View details" (same idea as child_removed)
                child_name: finalChildName,
                child_age: parseInt(finalChildAge, 10),
                relationship_to_parent: relationshipToParent,
                ...(finalChildEmail ? { child_email: finalChildEmail } : {}),
            }
        }, transaction);
...
const updateChild = async (req, res) => {
    ...
    try {
        ...
        let nextPricingType = child.pricing_type;
        let nextCustomAmount = child.custom_amount;

        if (subscriptionType) {
            const explicitPT =
                pricingType === 'custom' || pricingType === 'default' ? pricingType : null;
            const useCustom =
                explicitPT === 'custom' ||
                (explicitPT !== 'default' &&
                    customAmount != null &&
            ...
            if (useCustom) {
                monthlyAmount = parseFloat(customAmount);
                if (isNaN(monthlyAmount) || monthlyAmount <= 0) {
                    if (transaction) await transaction.rollback();
                    return res.status(400).json({
                        status: 'error',
                        message: 'Custom pricing requires a positive customAmount'
                    });
                }
                nextPricingType = 'custom';
                nextCustomAmount = monthlyAmount;
            } else {
                monthlyAmount = SUBSCRIPTION_PRICES[subscriptionType];
                nextPricingType = 'default';
                nextCustomAmount = null;
            }
        }
        ...
        await child.update({
            ...
            custom_amount: nextCustomAmount,
...
const removeChild = async (req, res) => {
    ...
    try {
        ...
        // Log activity before deletion.
        // IMPORTANT: we intentionally do NOT store child_id here because the
        // family_activity_log.child_id foreign key is ON DELETE CASCADE.
        // If we kept child_id, this "child_removed" log row would be deleted
        // together with the child record, and the Admin "Family Transactions"
        // screen would never show the removal event.
        await logFamilyActivity({
            family_id: familyId,
            // child_id omitted on purpose to preserve the log row after deletion
            user_id: req.user.id,
...
const getCartItems = async (req, res) => {
    try {
        ...
        cartItems.forEach(item => {
            ...
            const cartPricing = item.cart_pricing_type;
            const inferredPlanType =
                cartPricing ||
                (item.cart_custom_amount != null && parseFloat(item.cart_custom_amount) > 0
                    ? 'custom'
                    : item.cart_subscription_type
                      ? 'default'
                      : undefined);
...
const updateCartItemSubscription = async (req, res) => {
    try {
        const { familyId, childId } = req.params;
        const { subscriptionType, customAmount, planType, pricingType } = req.body;
        const salesUserId = req.user.id;
        ...
        const explicitPt = (planType || pricingType || '').toString().toLowerCase();
        const nextSub = subscriptionType || cartItem.cart_subscription_type;
        let nextCustom =
            customAmount !== undefined && customAmount !== null && customAmount !== ''
                ? (customAmount ? parseFloat(customAmount) : null)
                : cartItem.cart_custom_amount;
        let nextPricing = cartItem.cart_pricing_type;

        if (explicitPt === 'custom' || explicitPt === 'default') {
            nextPricing = explicitPt;
        }
        if (nextPricing === 'default') {
            nextCustom = null;
        }
        if (nextPricing === 'custom' && nextCustom == null && customAmount) {
            nextCustom = parseFloat(customAmount);
        }
        if (!nextPricing) {
            nextPricing =
                nextCustom != null && parseFloat(nextCustom) > 0
                    ? 'custom'
                    : nextSub
                      ? 'default'
                      : null;
        ...
        await cartItem.update({
            cart_subscription_type: nextSub,
            cart_custom_amount: nextCustom,
            cart_pricing_type: nextPricing
        });
    ...
    try {
        ...
        if (status_code === '000') {
            ...
            for (const childDetail of paymentLink.selected_children_details) {
                const childId = childDetail.child_id || childDetail.childId;
                const familyId = childDetail.family_id || childDetail.familyId;
                const childName = childDetail.child_name || childDetail.childName;
                const planRaw = (
                    childDetail.planType ||
                    childDetail.plan_type ||
                    childDetail.pricing_type ||
                    ''
                )
                    .toString()
                    .toLowerCase();
                ...
                if (planRaw === 'custom') {
                    isCustomPricing = true;
                } else if (planRaw === 'default') {
                    isCustomPricing = false;
                } else {
                    isCustomPricing = parseFloat(childDetail.custom_amount || 0) > 0;
                }
                const paidAmount = parseFloat(childDetail.amount || 0);

                await FamilyChild.update({
                    subscription_type: childDetail.subscription_type,
                    monthly_amount: paidAmount,
                    custom_amount: isCustomPricing ? paidAmount : null,
                    pricing_type: isCustomPricing ? 'custom' : 'default',
                    status: 'active',
                ...
                await logFamilyActivity({
                    ...
                    action_description: `Payment completed for ${childName} - ${childDetail.subscription_type} subscription`,
                    new_values: {
                        subscription_type: childDetail.subscription_type,
...
const bulkConfigureCartSubscriptions = async (req, res) => {
    ...
    try {
        transaction = await sequelize.transaction();
        const { configurations, childIds, subscriptionType, customAmount, planType, pricingType } = req.body;
        const salesUserId = req.user.id;
        ...
        if (Array.isArray(configurations) && configurations.length > 0) {
            for (const cfg of configurations) {
                const cid = cfg.childId;
                const subType = cfg.subscriptionType;
                if (!cid || !subType || !['monthly', 'quarterly', 'yearly'].includes(subType)) {
                    if (transaction) await transaction.rollback();
                    return res.status(400).json({
                        status: 'error',
                        message: 'Each configuration needs childId and valid subscriptionType'
                    });
                }
                const ptRaw = (cfg.planType || cfg.pricingType || 'default').toString().toLowerCase();
                const isCustom = ptRaw === 'custom';
                const amtRaw = cfg.customAmount ?? cfg.amount;
                const amt =
                    amtRaw !== undefined && amtRaw !== null && amtRaw !== ''
                        ? parseFloat(amtRaw)
                        : null;
                if (isCustom && (amt == null || isNaN(amt) || amt <= 0)) {
                    if (transaction) await transaction.rollback();
                    return res.status(400).json({
                        status: 'error',
                        message: `Custom plan requires a positive amount for child ${cid}`
                    });
                }
                const [n] = await FamilyCartItem.update(
                    {
                        cart_subscription_type: subType,
                        cart_custom_amount: isCustom ? amt : null,
                        cart_pricing_type: isCustom ? 'custom' : 'default'
                    },
                    {
                        where: {
                            sales_user_id: salesUserId,
            ...
            const ptRaw = (planType || pricingType || (customAmount ? 'custom' : 'default'))
                .toString()
                .toLowerCase();
            const isCustom = ptRaw === 'custom';
            ...
            const updatedCount = await FamilyCartItem.update(
                {
                    ...
                    cart_custom_amount: isCustom && customAmount ? parseFloat(customAmount) : null,
                    cart_pricing_type: isCustom ? 'custom' : 'default'
                },
                {
                    where: {
        ...
        return res.status(200).json({
            ...
            message: `Bulk configured ${updatedTotal} cart items`
        });
