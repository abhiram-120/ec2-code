...
const { processFamilyFailedWebhookPayment } = require('./family-payment-failed.controller');
...
const processPayPlusWebhook = async (req, res) => {
  ...
  try {
    ...
    const existingSuccessfulTransaction = await PaymentTransaction.findOne({
      where: {
        [Op.or]: [
    ...
    let existingRecurringFamilyTransaction = null;
    if (isRecurringCharge) {
      existingRecurringFamilyTransaction = await FamilyPaymentTransaction.findOne({
        where: {
    ...
    if (existingWebhook || existingSuccessfulTransaction || existingRecurringFamilyTransaction) {
      const duplicateSource = existingWebhook
        ? 'webhook_log'
        : (existingSuccessfulTransaction ? 'payment_transaction' : 'family_recurring_transaction');

      console.log(`⚠️ Duplicate webhook ignored for transaction ${transaction_uid} (source: ${duplicateSource})`);
      ...
      familyPaymentLogger.logFamilyWebhookEvent({
        ...
        error_details: {
          ...
          existing_payment_transaction_id: existingSuccessfulTransaction?.id || null,
          existing_family_transaction_id: existingRecurringFamilyTransaction?.id || null
        },
        processing_result: {
...
const processFamilyRecurringPayment = async (webhookData, webhookLogId, transaction) => {
  try {
      ...
      try {
        ...
        if (subscriptionResult.success) {
          processedChildren.push({
            ...
            childName: familyChild.child_name,
            familyId: familyChild.family_id,
            relationship: familyChild.relationship_to_parent, // Add relationship from FamilyChild
    ...
    await resolveFamilyPastDuePayments(
      familyId,
      transaction_uid,
      parseFloat(totalProcessedAmount || amount || 0),
      currency_code || 'ILS',
      transaction
    );


    let familyRecurringTransaction = await FamilyPaymentTransaction.findOne({
      where: {
        payplus_transaction_id: transaction_uid,
        family_id: familyId
      },
    ...
    if (!familyRecurringTransaction) {
      const recurringTransactionData = {
        ...
        paid_children_details: processedChildren.map(pc => ({
          ...
          relationship: pc.relationship, // Include relationship in paid_children_details
          userId: pc.userId,
          amount: pc.amount,
      ...
      familyRecurringTransaction = await FamilyPaymentTransaction.create(recurringTransactionData, { transaction });
      console.log(`Created family recurring transaction: ${familyRecurringTransaction.id}`);
    } else {
      console.log(`Found existing family recurring transaction for ${transaction_uid}: ${familyRecurringTransaction.id}, skipping duplicate create.`);
    }
...
const renewChildSubscription = async (userId, childData, transactionUid, transaction) => {
  try {
    ...
    // Calculate subscription values using child duration months
    const subscriptionMonths = parseInt(childData.durationMonths, 10) || 1;
    ...
    if (currentSubscription) {
      // ✅ CASE 1: Active subscription exists - deactivate and create a new one
      console.log(`📋 Found current subscription: ${currentSubscription.id} - deactivating and creating new subscription`);

      await currentSubscription.update({
        status: 'inactive',
        is_cancel: 1,
        updated_at: new Date(),
        notes: `${currentSubscription.notes || ''}\n\nRecurring payment renewal. Previous subscription deactivated. Transaction: ${transactionUid}. Deactivated: ${new Date().toISOString()}.`
      }, { transaction });

      console.log(`📋 Deactivated existing subscription: ${currentSubscription.id} for user ${userId}`);
    } else {
...
/**
 * Resolve active family past-due records and disable dunning after successful payment.
 * This should not block the main payment flow if resolution fails.
 */
const resolveFamilyPastDuePayments = async (familyId, transactionUid, amount, currency, transaction) => {
  if (!familyId) return;

  try {
    const activePastDuePayments = await FamilyPastDuePayment.findAll({
      where: {
        family_id: familyId,
        status: 'past_due'
      },
    ...
    if (!activePastDuePayments.length) {
      return;
    }
    ...
    for (const pastDuePayment of activePastDuePayments) {
      await pastDuePayment.update({
        ...
        resolved_payment_method: 'payplus_family_auto_resolve',
        notes: `${pastDuePayment.notes || ''}\n[${now.toISOString()}] Auto-resolved after successful family payment. Transaction: ${transactionUid}. Amount: ${amount} ${currency}.`
      }, { transaction });
    ...
    paymentLogger.logPaymentVerification({
      student_id: `family_${familyId}`,
      student_name: 'Family',
      subscription_id: null,
      verification_type: 'family_past_due_resolved_after_payment',
      verification_result: true,
      subscription_details: {
        family_id: familyId,
        resolved_count: activePastDuePayments.length,
        transaction_uid: transactionUid,
        amount,
        currency
      }
    ...
    paymentLogger.logPaymentVerification({
      ...
      student_name: 'Family',
      subscription_id: null,
      verification_type: 'family_past_due_resolve_error',
      verification_result: false,
      error_details: {
        error_type: 'family_past_due_resolution_error',
        error_message: error.message,
        error_stack: error.stack,
        family_id: familyId,
        transaction_uid: transactionUid
...
const processSuccessfulWebhookPayment = async (webhookData, webhookLogId, transaction) => {
    try {
        ...
        const familyId = additionalData.family_id
            ? parseInt(additionalData.family_id)
            : (additionalData.familyId ? parseInt(additionalData.familyId) : null);
...
const getPaymentTransactions = async (req, res) => {
  try {
    const { student_id, status, generated_by, search = '', page = 1, limit = 20 } = req.query;
    ...
    if (trimmedSearch) {
      andConditions.push({
        [Op.or]: [
          { transaction_id: { [Op.like]: `%${trimmedSearch}%` } },
          { student_name: { [Op.like]: `%${trimmedSearch}%` } },
          { student_email: { [Op.like]: `%${trimmedSearch}%` } },
