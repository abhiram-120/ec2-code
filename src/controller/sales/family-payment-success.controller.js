...
const processFamilyPaymentSuccess = async (req, res) => {
  ...
  try {
    ...
    if (existingTransaction) {
      console.log('Transaction already processed, fetching children and family data...');
      
      // Extract children data from existing transaction
      let childrenData = [];
      let familiesData = [];
      
      try {
        // Get paid_children_details from transaction
        const paidChildrenDetails = existingTransaction.paid_children_details;
        
        if (paidChildrenDetails) {
          let parsedChildren = [];
          
          // Parse if it's a string
          if (typeof paidChildrenDetails === 'string') {
            parsedChildren = JSON.parse(paidChildrenDetails);
          } else if (Array.isArray(paidChildrenDetails)) {
            parsedChildren = paidChildrenDetails;
          } else if (typeof paidChildrenDetails === 'object') {
            parsedChildren = [paidChildrenDetails];
          }

          // Format children data for response
          childrenData = parsedChildren.map(child => ({
            childId: child.childId || child.child_id || null,
            childName: child.childName || child.child_name || 'Child',
            childEmail: child.childEmail || child.child_email || null,
            familyId: child.familyId || child.family_id || null,
            parentName: child.parentName || child.parent_name || '',
            parentEmail: child.parentEmail || child.parent_email || '',
            planType: child.planType || child.plan_type || 'default',
            durationMonths: child.durationMonths || child.duration_months || 1,
            lessonMinutes: child.lessonMinutes || child.lesson_minutes || 25,
            lessonsPerMonth: child.lessonsPerMonth || child.lessons_per_month || 4,
            amount: child.amount || 0,
            planDescription: child.planDescription || child.plan_description || 'Learning Plan'
          }));

          console.log(`Extracted ${childrenData.length} children from existing transaction`);

          // Get unique family IDs from children
          const familyIds = [...new Set(childrenData.map(c => c.familyId).filter(Boolean))];
          
          // Fetch families data separately (Family is not associated with FamilyPaymentLink)
          if (familyIds.length > 0) {
            ...
            console.log(`Extracted ${familiesData.length} families from existing transaction`);
          }
        }
      } catch (error) {
        console.error('Error extracting children/family data from existing transaction:', error);
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          status: 'success',
          family_payment_transaction_id: existingTransaction.id,
          children: childrenData,
          families: familiesData,
          children_count: childrenData.length,
          families_count: familiesData.length,
          registered_students_count: existingTransaction.student_ids?.length || 0
        },
    ...
    try {
      webhookResult = await processFamilyPaymentWebhook(normalizedData, null, transaction);
    ...
    // Prepare response data with family and child information
    const responseData = {
      transaction_id: transactionId,
      status: 'success',
      link_token: linkToken,
    ...
    // Add family and child data if available
    if (webhookResult) {
      responseData.family_payment_transaction_id = webhookResult.familyPaymentTransaction?.id;
      responseData.families = webhookResult.families || [];
      responseData.children = (webhookResult.selectedChildren || []).map(child => ({
        childId: child.childId,
        childName: child.childName,
        childEmail: child.childEmail || null,
        familyId: child.familyId,
        parentName: child.parentName,
        parentEmail: child.parentEmail,
        planType: child.planType,
        durationMonths: child.durationMonths,
        lessonMinutes: child.lessonMinutes,
        lessonsPerMonth: child.lessonsPerMonth,
        amount: child.amount,
        planDescription: child.planDescription
      }));
      responseData.children_count = webhookResult.selectedChildren?.length || 0;
      responseData.families_count = webhookResult.families?.length || 0;
      responseData.registered_students_count = webhookResult.registeredStudentIds?.length || 0;
    }
  ...
  try {
    ...
    for (const familyChild of familyChildren) {
      try {
        ...
        // Derive child-specific phone from parent's phone (similar to email alias)
        let childMobile = null;
        if (family.parent_phone) {
          const basePhone = family.parent_phone.toString().trim();
          if (basePhone.length > 0) {
            const safeChildName = (familyChild.child_name || '').replace(/\s+/g, '');
            childMobile = `${basePhone}+${safeChildName}`;
          }
        }
        ...
        const userData = {
          ...
          verified: true,
          language: 'EN',
          // Child-specific phone alias based on parent's phone + child name
          mobile: childMobile,
          country_code: family.parent_country_code || null,
          created_at: Math.floor(Date.now() / 1000),
          password: hashedPassword,
    ...
    for (const familyChild of familyChildren) {
      ...
      const planRaw = (
        ...
        .toString()
        .toLowerCase();
      const isCustomPricing = planRaw === 'custom';
      const paidAmount = parseFloat(
        childDetail.amount || familyChild.monthly_amount || 0
      );
    ...
    // Return family and child data for the response
    return {
