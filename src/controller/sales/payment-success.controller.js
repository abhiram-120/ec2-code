...
const processSuccessfulWebhookPayment = async (webhookData, webhookLogId, transaction) => {
    try {
                    ...
                    if (base64Decoded && base64Decoded.trim().length > 0) {
                        ...
                        if (cleanedJson.startsWith('{') && cleanedJson.endsWith('}')) {
                            ...
                            additionalData = {
                                ...
                                // gto: existing-user guided trial 89 ILS one-time (mobile) — payment only, no new subscription
                                guided_trial_one_time:
                                    decodedData.gto === true ||
                                    decodedData.gto === 'true' ||
                                    decodedData.guided_trial_one_time === true
                            };
        ...
        if (trialClassId && !isNaN(trialClassId) && trialClassId > 0 && additionalData.payment_type !== 'existing_user' && additionalData.payment_type !== 'mobile_guided_trial') {
