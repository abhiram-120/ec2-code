// controller/sales/family-payment-failed.controller.js
const { Family, FamilyChild, FamilyPaymentLink, FamilyPaymentTransaction, FamilyActivityLog } = require('../../models/Family');
const FamilyPastDuePayment = require('../../models/FamilyPastDuePayment');
const User = require('../../models/users');
const UserSubscriptionDetails = require('../../models/UserSubscriptionDetails');
const PayPlusWebhookLog = require('../../models/PayPlusWebhookLog');
const RecurringPayment = require('../../models/RecurringPayment');
const FamilyDunningSchedule = require('../../models/FamilyDunningSchedule');
const { cancelFamilyRecurringPayment } = require('../../services/familyPayplus.service');
const { paymentLogger } = require('../../utils/paymentLogger');
const { sequelize } = require('../../connection/connection');
const { Sequelize } = require('sequelize');
const { sendCombinedNotifications } = require('../../cronjobs/reminder');
const { Op } = require('sequelize');
const moment = require('moment-timezone');
const axios = require('axios');
const crypto = require('crypto');

/**
 * Generate short ID for family past due payments (8-char hex, uppercase)
 */
const generateShortId = () => crypto.randomBytes(4).toString('hex').toUpperCase();

/**
 * Generate a unique transaction ID when one is missing
 * @returns {String} - Unique transaction ID
 */
const generateTransactionId = () => {
  return `fam_failed_txn_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
};

/**
 * Parse the PayPlus response data with enhanced error handling and debugging
 * @param {Object} body - Request body from PayPlus
 * @returns {Object} - Parsed data
 */
const parsePayPlusResponse = (body) => {
  try {
    console.log('=== parsePayPlusResponse DEBUG (FAILED) ===');
    console.log('Input body type:', typeof body);
    console.log('Input body:', JSON.stringify(body, null, 2));
    console.log('Body keys count:', Object.keys(body || {}).length);

    // Handle empty or null body
    if (!body || typeof body !== 'object') {
      console.log('Invalid body - returning empty object');
      return {};
    }

    // If body contains a single key with PayPlus's format (starting with &)
    if (Object.keys(body).length === 1 && Object.keys(body)[0].startsWith('&')) {
      console.log('Detected legacy PayPlus format - parsing...');
      
      const responseString = Object.keys(body)[0];
      const params = {};
      const pairs = responseString.substring(1).split('&'); // Remove the leading & and split

      pairs.forEach(pair => {
        const [key, value] = pair.split('=');
        if (key && value !== undefined) {
          params[key] = decodeURIComponent(value);
        }
      });

      // Extract data from original_data if it exists
      if (params.original_data) {
        try {
          const decodedData = atob(params.original_data);
          const originalParams = new URLSearchParams(decodedData);

          // Merge original_data parameters into our result
          originalParams.forEach((value, key) => {
            if (!params[key]) {
              params[key] = value;
            }
          });
        } catch (error) {
          console.log('Error decoding original_data:', error.message);
          paymentLogger.logWebhookEvent({
            event_type: 'original_data_decode_error',
            transaction_uid: 'unknown',
            status: 'warning',
            amount: null,
            currency: null,
            customer_email: null,
            customer_name: null,
            payment_method: null,
            error_details: {
              error_type: 'decode_error',
              error_message: error.message,
              original_data_length: params.original_data?.length || 0
            }
          });
        }
      }

      console.log('Legacy format parsed result:', JSON.stringify(params, null, 2));
      return params;
    }

    // If it's already in a structured format, return it as is
    console.log('Detected structured format - returning as-is');
    console.log('Returning body with keys:', Object.keys(body));
    
    // Ensure all expected fields are present (case-insensitive check)
    const normalizedBody = {};
    Object.keys(body).forEach(key => {
      // Keep original key
      normalizedBody[key] = body[key];
      
      // Also create lowercase version for compatibility
      const lowerKey = key.toLowerCase();
      if (lowerKey !== key && !normalizedBody[lowerKey]) {
        normalizedBody[lowerKey] = body[key];
      }
    });

    console.log('Normalized body keys:', Object.keys(normalizedBody));
    return normalizedBody;

  } catch (error) {
    console.log('Error in parsePayPlusResponse:', error);
    
    paymentLogger.logWebhookEvent({
      event_type: 'family_payplus_failed_response_parse_error',
      transaction_uid: 'unknown',
      status: 'error',
      amount: null,
      currency: null,
      customer_email: null,
      customer_name: null,
      payment_method: null,
      error_details: {
        error_type: 'parse_error',
        error_message: error.message,
        body_keys: Object.keys(body || {}),
        body_type: typeof body
      }
    });
    
    console.log('Returning original body due to error');
    return body || {};
  }
};

/**
 * Robust link token extraction with multiple fallback strategies
 * @param {Object} parsedData - Parsed PayPlus response data  
 * @param {Object} originalBody - Original request body as fallback
 * @returns {String} - Extracted link token or empty string
 */
const extractLinkToken = (parsedData, originalBody = {}) => {
  console.log('=== ROBUST TOKEN EXTRACTION (FAILED PAYMENT) ===');
  
  // Strategy 1: Direct extraction from parsedData
  let token = parsedData?.more_info_1;
  if (token && typeof token === 'string' && token.trim().length > 0) {
    console.log('Token found via Strategy 1 (parsedData.more_info_1):', token);
    return token.trim();
  }

  // Strategy 2: Case-insensitive field search in parsedData
  const fieldVariations = [
    'more_info_1', 'More_Info_1', 'MORE_INFO_1', 'more-info-1', 'moreInfo1',
    'more_info1', 'linkToken', 'link_token', 'token'
  ];
  
  for (const field of fieldVariations) {
    token = parsedData?.[field];
    if (token && typeof token === 'string' && token.trim().length > 0) {
      console.log(`Token found via Strategy 2 (${field}):`, token);
      return token.trim();
    }
  }

  // Strategy 3: Deep search in nested objects (like params)
  if (typeof parsedData === 'object') {
    for (const [key, value] of Object.entries(parsedData)) {
      if (typeof value === 'object' && value !== null) {
        for (const field of fieldVariations) {
          token = value[field];
          if (token && typeof token === 'string' && token.trim().length > 0) {
            console.log(`Token found via Strategy 3 (${key}.${field}):`, token);
            return token.trim();
          }
        }
      }
    }
  }

  // Strategy 4: Try original body if parsedData failed
  if (originalBody && typeof originalBody === 'object') {
    for (const field of fieldVariations) {
      token = originalBody[field];
      if (token && typeof token === 'string' && token.trim().length > 0) {
        console.log(`Token found via Strategy 4 (originalBody.${field}):`, token);
        return token.trim();
      }
    }
  }

  console.log('No token found with any strategy');
  return '';
};

/**
 * Process failed family payment callback from PayPlus - COMPLETE VERSION
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
const processFamilyPaymentFailed = async (req, res) => {
  let transaction;
  const processingStartTime = Date.now();
  
  // Add request timeout to prevent hanging
  const timeout = setTimeout(() => {
    console.error('Family failed payment processing timeout - force responding');
    if (!res.headersSent) {
      res.status(408).json({
        status: 'error',
        message: 'Failed payment processing timeout',
        details: 'Processing took longer than expected'
      });
    }
  }, 30000); // 30 second timeout

  try {
    console.log('Starting family failed payment processing...');
    
    // Start database transaction
    transaction = await sequelize.transaction();
    console.log('Database transaction started');

    // Parse the PayPlus response data
    const parsedData = parsePayPlusResponse(req.body);
    console.log('Failed payment data parsed successfully:', parsedData);

    // Log family failed callback processing start
    paymentLogger.logWebhookEvent({
      event_type: 'family_callback_failed',
      transaction_uid: 'parsing',
      status: 'processing_failed',
      amount: null,
      currency: null,
      customer_email: null,
      customer_name: null,
      payment_method: null,
      processing_result: {
        callback_type: 'family_failed',
        processing_start_time: new Date().toISOString(),
        user_agent: req.headers['user-agent'],
        ip_address: req.ip
      },
      webhook_payload: req.body
    });

    console.log('Initial webhook event logged');

    // Extract essential data - PayPlus specific field mappings with fallbacks
    const transactionId = parsedData.transaction_uid ||
      parsedData.transaction_id ||
      parsedData.index ||
      generateTransactionId();

    console.log('Failed transaction ID extracted:', transactionId);

    // Validate transaction ID
    if (!transactionId || transactionId === 'undefined' || transactionId === '') {
      console.error('Invalid transaction ID');
      
      paymentLogger.logWebhookEvent({
        event_type: 'family_callback_failed_validation',
        transaction_uid: transactionId || 'invalid',
        status: 'validation_failed',
        amount: parseFloat(parsedData.amount || parsedData.sum || 0),
        currency: parsedData.currency_code || parsedData.currency || 'ILS',
        customer_email: parsedData.customer_email || parsedData.email || '',
        customer_name: parsedData.customer_name || parsedData.contact || '',
        payment_method: parsedData.payment_method || parsedData.cardtype || '',
        error_details: {
          error_type: 'invalid_transaction_id',
          error_message: 'Missing or invalid transaction ID parameters',
          provided_transaction_id: transactionId
        }
      });

      clearTimeout(timeout);
      if (transaction) await transaction.rollback();
      return res.status(400).json({
        status: 'error',
        message: 'Missing or invalid transaction ID parameters'
      });
    }

    // Extract link token with robust method
    const linkToken = extractLinkToken(parsedData, req.body);
    console.log('Link token extracted:', linkToken);

    if (!linkToken) {
      console.error('Missing link token for failed payment');
      
      paymentLogger.logWebhookEvent({
        event_type: 'family_callback_failed_validation',
        transaction_uid: transactionId,
        status: 'validation_failed',
        amount: parseFloat(parsedData.amount || parsedData.sum || 0),
        currency: parsedData.currency_code || parsedData.currency || 'ILS',
        customer_email: parsedData.customer_email || parsedData.email || '',
        customer_name: parsedData.customer_name || parsedData.contact || '',
        payment_method: parsedData.payment_method || parsedData.cardtype || '',
        error_details: {
          error_type: 'missing_link_token',
          error_message: 'Missing family payment link token for failed payment'
        }
      });

      clearTimeout(timeout);
      if (transaction) await transaction.rollback();
      return res.status(400).json({
        status: 'error',
        message: 'Missing family payment link token'
      });
    }

    console.log('Checking for existing failed transactions...');

    // Check if this failed transaction has already been processed
    const existingFailedTransaction = await FamilyPaymentTransaction.findOne({
      where: {
        [Op.or]: [
          { payplus_transaction_id: transactionId },
          { transaction_token: `fam_failed_${transactionId}` }
        ],
        status: 'failed'
      },
      transaction
    });

    console.log('Existing failed transaction check completed:', !!existingFailedTransaction);

    if (existingFailedTransaction) {
      console.log('Failed transaction already processed');
      
      paymentLogger.logWebhookEvent({
        event_type: 'family_callback_failed_duplicate',
        transaction_uid: transactionId,
        status: 'duplicate',
        amount: parseFloat(parsedData.amount || parsedData.sum || 0),
        currency: parsedData.currency_code || parsedData.currency || 'ILS',
        customer_email: parsedData.customer_email || parsedData.email || '',
        customer_name: parsedData.customer_name || parsedData.contact || '',
        payment_method: parsedData.payment_method || parsedData.cardtype || '',
        processing_result: {
          duplicate_prevented: true,
          existing_transaction_id: existingFailedTransaction.id
        }
      });

      clearTimeout(timeout);
      if (transaction) await transaction.rollback();
      return res.status(200).json({
        status: 'success',
        data: {
          transaction_id: transactionId,
          status: 'failed'
        },
        message: 'Family failed payment already processed'
      });
    }

    console.log('Normalizing data for failed webhook processing...');

    // Normalize the parsed data to family payment format
    const normalizedData = {
      transaction_uid: transactionId,
      amount: parseFloat(parsedData.amount || parsedData.sum || 0),
      currency_code: parsedData.currency_code || parsedData.currency || 'ILS',
      customer_name: parsedData.customer_name || parsedData.contact || '',
      customer_email: parsedData.customer_email || parsedData.email || '',
      payment_method: parsedData.payment_method || parsedData.cardtype || '',
      four_digits: parsedData.four_digits || parsedData.ccno || '',
      status_code: parsedData.status_code || parsedData.error_code || parsedData.Response || 'unknown_error',
      more_info_1: linkToken,
      more_info_2: parsedData.more_info_2 || parsedData.children_count || '',
      more_info_3: parsedData.more_info_3 || parsedData.families_count || '',
      more_info_4: parsedData.more_info_4 || parsedData.short_id || '',
      more_info_5: parsedData.more_info_5 || '',
      is_recurring: parsedData.recurring || parsedData.charge_method === '3' || false,
      recurring_info: {},
      original_webhook: parsedData
    };

    console.log('Data normalized successfully');
    console.log('Normalized failed data summary:', {
      transaction_uid: normalizedData.transaction_uid,
      amount: normalizedData.amount,
      currency: normalizedData.currency_code,
      link_token: normalizedData.more_info_1,
      customer: normalizedData.customer_name,
      error_code: normalizedData.status_code
    });

    console.log('Starting family failed payment webhook processing...');

    // Process the family failed payment with proper error handling
    try {
      await processFamilyPaymentFailedWebhook(normalizedData, null, transaction);
      console.log('Family failed payment webhook processing completed');
    } catch (webhookError) {
      console.error('Error in processFamilyPaymentFailedWebhook:', webhookError);
      
      // Log webhook processing error
      paymentLogger.logWebhookEvent({
        event_type: 'family_callback_failed_webhook_error',
        transaction_uid: transactionId,
        status: 'error',
        amount: normalizedData.amount,
        currency: normalizedData.currency_code,
        customer_email: normalizedData.customer_email,
        customer_name: normalizedData.customer_name,
        payment_method: normalizedData.payment_method,
        error_details: {
          error_type: 'failed_webhook_processing_error',
          error_message: webhookError.message,
          error_stack: webhookError.stack
        }
      });

      throw webhookError; // Re-throw to be caught by main error handler
    }

    console.log('Committing database transaction...');
    await transaction.commit();
    console.log('Database transaction committed successfully');

    const processingTime = Date.now() - processingStartTime;

    // Log successful family failed processing
    paymentLogger.logWebhookEvent({
      event_type: 'family_callback_failed_complete',
      transaction_uid: transactionId,
      status: 'failed',
      amount: normalizedData.amount,
      currency: normalizedData.currency_code,
      customer_email: normalizedData.customer_email,
      customer_name: normalizedData.customer_name,
      payment_method: normalizedData.payment_method,
      error_details: {
        payment_failure_code: normalizedData.status_code,
        payment_failure_reason: 'Family payment failed at processor level'
      },
      processing_result: {
        processing_time_ms: processingTime,
        family_failed_callback_processed: true,
        failure_recorded: true,
        link_token: linkToken,
        payment_link_kept_active: true
      }
    });

    console.log(`Family failed payment processing completed successfully in ${processingTime}ms`);

    clearTimeout(timeout);

    return res.status(200).json({
      status: 'success',
      data: {
        transaction_id: transactionId,
        status: 'failed',
        link_token: linkToken,
        processing_time_ms: processingTime,
        retry_available: true
      },
      message: 'Family failed payment processed successfully'
    });

  } catch (error) {
    console.error('Critical error in processFamilyPaymentFailed:', error);

    if (transaction) {
      try {
        console.log('Rolling back database transaction...');
        await transaction.rollback();
        console.log('Database transaction rolled back successfully');
      } catch (rollbackError) {
        console.error('Error rolling back transaction:', rollbackError);
        
        paymentLogger.logWebhookEvent({
          event_type: 'family_failed_transaction_rollback_error',
          transaction_uid: 'unknown',
          status: 'error',
          amount: null,
          currency: null,
          customer_email: null,
          customer_name: null,
          payment_method: null,
          error_details: {
            error_type: 'rollback_error',
            error_message: rollbackError.message,
            error_stack: rollbackError.stack
          }
        });
      }
    }

    const processingTime = Date.now() - processingStartTime;

    // Log family failed processing error
    paymentLogger.logWebhookEvent({
      event_type: 'family_callback_failed_error',
      transaction_uid: 'unknown',
      status: 'error',
      amount: null,
      currency: null,
      customer_email: null,
      customer_name: null,
      payment_method: null,
      error_details: {
        error_type: 'family_failed_processing_error',
        error_message: error.message,
        error_stack: error.stack,
        processing_time_ms: processingTime
      },
      webhook_payload: req.body
    });

    clearTimeout(timeout);

    // Ensure we send a response
    if (!res.headersSent) {
      return res.status(500).json({
        status: 'error',
        message: 'Error processing family failed payment',
        details: error.message,
        processing_time_ms: processingTime
      });
    }
  }
};

/**
 * Process family payment failed webhook - handles family-specific failure logic
 * @param {Object} webhookData - Extracted PayPlus webhook data
 * @param {Number} webhookLogId - ID of the webhook log entry (can be null)
 * @param {Object} transaction - Database transaction
 */
const processFamilyPaymentFailedWebhook = async (webhookData, webhookLogId, transaction) => {
  try {
    const {
      transaction_uid,
      amount,
      currency_code,
      customer_name,
      customer_email,
      payment_method,
      four_digits,
      status_code,
      more_info_1: linkToken,
      more_info_2: childrenCount,
      more_info_3: familiesCount,
      more_info_4: shortId,
      more_info_5: encodedData,
      is_recurring,
      recurring_info
    } = webhookData;

    console.log(`Processing family failed payment: ${transaction_uid} for link: ${linkToken}`);

    // Log family payment verification start
    paymentLogger.logPaymentVerification({
      student_id: linkToken || 'unknown',
      student_name: customer_name || 'unknown',
      subscription_id: null,
      verification_type: 'family_failed_payment_processing',
      verification_result: false,
      error_details: {
        transaction_uid: transaction_uid,
        failure_reason: `Payment failed with status: ${status_code}`,
        amount: amount,
        currency: currency_code
      }
    });

    // Validate required fields
    if (!transaction_uid || transaction_uid === 'undefined' || transaction_uid === '') {
      throw new Error('Transaction UID is required for family failed payment processing');
    }

    // SPECIAL CASE: recurring family failures where PayPlus does NOT send a link token
    // For these, we derive family/children from items and trigger dunning directly.
    if (is_recurring && (!linkToken || linkToken === 'undefined' || linkToken === '')) {
      console.log('[DUNNING] Recurring family failed payment without link token - processing via items');
      await processFamilyRecurringFailureWithoutLink(webhookData, transaction);
      return;
    }

    if (!linkToken || linkToken === 'undefined' || linkToken === '') {
      throw new Error('Link token is required for family failed payment processing');
    }

    // Find the family payment link
    const familyPaymentLink = await FamilyPaymentLink.findOne({
      where: { link_token: linkToken },
      transaction
    });

    if (!familyPaymentLink) {
      paymentLogger.logPaymentVerification({
        student_id: linkToken,
        student_name: customer_name || 'unknown',
        subscription_id: null,
        verification_type: 'family_failed_payment_link_lookup',
        verification_result: false,
        error_details: {
          message: 'Family payment link not found for failed payment',
          link_token: linkToken
        }
      });
      throw new Error(`Family payment link not found for token: ${linkToken}`);
    }

    console.log(`Found family payment link: ${familyPaymentLink.id}`);

    // SAFE JSON PARSING: Handle selected_children_details properly for failed payment
    let selectedChildren = [];
    
    try {
      const rawChildrenData = familyPaymentLink.selected_children_details;
      console.log('Raw children data type for failed payment:', typeof rawChildrenData);
      console.log('Raw children data:', rawChildrenData);
      
      if (rawChildrenData) {
        if (typeof rawChildrenData === 'string') {
          // Parse JSON string
          selectedChildren = JSON.parse(rawChildrenData);
          console.log('Parsed children data from JSON string');
        } else if (Array.isArray(rawChildrenData)) {
          // Already an array
          selectedChildren = rawChildrenData;
          console.log('Children data already an array');
        } else if (typeof rawChildrenData === 'object') {
          // Convert object to array if possible
          if (rawChildrenData.length !== undefined) {
            selectedChildren = Array.from(rawChildrenData);
            console.log('Converted object to array');
          } else {
            // Single object, wrap in array
            selectedChildren = [rawChildrenData];
            console.log('Wrapped single object in array');
          }
        } else {
          console.error('Unexpected children data type:', typeof rawChildrenData);
          selectedChildren = [];
        }
      }
      
      console.log('Final selectedChildren type:', typeof selectedChildren);
      console.log('Final selectedChildren is array:', Array.isArray(selectedChildren));
      console.log('Final selectedChildren length:', selectedChildren.length);
      
    } catch (parseError) {
      console.error('Error parsing selected_children_details for failed payment:', parseError);
      console.log('Raw data that failed to parse:', familyPaymentLink.selected_children_details);
      
      paymentLogger.logPaymentVerification({
        student_id: linkToken,
        student_name: customer_name || 'unknown',
        subscription_id: null,
        verification_type: 'family_failed_children_data_parse_error',
        verification_result: false,
        error_details: {
          error_type: 'json_parse_error',
          error_message: parseError.message,
          raw_data: familyPaymentLink.selected_children_details
        }
      });
      
      // Try to recover by creating a minimal structure
      selectedChildren = [];
    }

    // Ensure selectedChildren is an array
    if (!Array.isArray(selectedChildren)) {
      console.error('selectedChildren is not an array after processing:', typeof selectedChildren);
      selectedChildren = [];
    }

    if (selectedChildren.length === 0) {
      console.warn('No children found in family payment link after parsing - proceeding with minimal data');
    }

    console.log(`Processing failed payment for ${selectedChildren.length} children`);

    // Parse additional encoded data if available
    let additionalFamilyData = {};
    if (encodedData && encodedData !== '' && encodedData !== 'undefined') {
      try {
        const urlDecoded = decodeURIComponent(encodedData);
        const base64Decoded = Buffer.from(urlDecoded, 'base64').toString('utf8');
        additionalFamilyData = JSON.parse(base64Decoded);
        console.log('Successfully parsed additional family data for failed payment:', Object.keys(additionalFamilyData));
      } catch (error) {
        console.warn('Failed to parse additional family data for failed payment:', error.message);
        additionalFamilyData = {};
      }
    }

    // Extract message_description from webhook (same as PayPlus sends)
    let messageDescription = `Family payment failed with status: ${status_code}`;
    const originalWebhook = webhookData.original_webhook || webhookData;
    if (originalWebhook?.transaction?.message_description) {
      messageDescription = originalWebhook.transaction.message_description;
    } else if (webhookData?.transaction?.message_description) {
      messageDescription = webhookData.transaction.message_description;
    } else if (originalWebhook?.message_description) {
      messageDescription = originalWebhook.message_description;
    } else if (webhookData?.message_description) {
      messageDescription = webhookData.message_description;
    }

    // Extract child IDs and build processedChildren structure (similar to success flow)
    let paidChildrenIds = [];
    const processedChildren = []; // Similar to success flow - track processed children with user/subscription info
    const foundSubscriptionIds = []; // Similar to success flow - track subscription IDs
    
    if (selectedChildren && selectedChildren.length > 0) {
      const childIds = selectedChildren
        .map(child => child.childId || child.child_id || child.id)
        .filter(id => id != null);
      
      paidChildrenIds = childIds;
      
      // Process each child to find user and subscriptions (similar to success flow pattern)
      if (childIds.length > 0) {
        try {
          const familyChildren = await FamilyChild.findAll({
            where: {
              id: { [Op.in]: childIds },
              family_id: selectedChildren[0]?.familyId || null
            },
            attributes: ['id', 'child_name', 'child_email', 'family_id'],
            transaction
          });
          
          // Process each child individually (like success flow)
          for (const familyChild of familyChildren) {
            try {
              let existingUser = null;
              
              // Look up User by child email (same pattern as success flow)
              if (familyChild.child_email) {
                existingUser = await User.findOne({
                  where: { email: familyChild.child_email },
                  attributes: ['id', 'email', 'full_name'],
                  transaction
                });
                
                if (existingUser) {
                  console.log(`[FAILED PAYMENT] Found user by child email: ${existingUser.email} for child ${familyChild.id}`);
                }
              }
              
              // Fallback: try generated email pattern (same as success flow)
              if (!existingUser && selectedChildren[0]?.familyId) {
                const family = await Family.findByPk(selectedChildren[0].familyId, {
                  attributes: ['parent_email'],
                  transaction
                });
                
                if (family?.parent_email) {
                  const emailParts = family.parent_email.split('@');
                  if (emailParts.length === 2) {
                    const generatedEmail = `${emailParts[0]}+familychild${familyChild.id}@${emailParts[1]}`;
                    existingUser = await User.findOne({
                      where: { email: generatedEmail },
                      attributes: ['id', 'email', 'full_name'],
                      transaction
                    });
                    
                    if (existingUser) {
                      console.log(`[FAILED PAYMENT] Found user by generated email pattern: ${existingUser.email}`);
                    }
                  }
                }
              }
              
              // Fallback: try by name and account type (same as success flow)
              if (!existingUser) {
                existingUser = await User.findOne({
                  where: { 
                    full_name: familyChild.child_name,
                    account_type: 'family_child'
                  },
                  attributes: ['id', 'email', 'full_name'],
                  transaction
                });
                
                if (existingUser) {
                  console.log(`[FAILED PAYMENT] Found user by name and account type: ${existingUser.full_name}`);
                }
              }
              
              if (existingUser) {
                // Find subscriptions for this user (similar to success flow)
                const userSubscriptions = await UserSubscriptionDetails.findAll({
                  where: {
                    user_id: existingUser.id,
                    status: { [Op.ne]: 'inactive' } // Only active subscriptions
                  },
                  attributes: ['id', 'user_id'],
                  transaction
                });
                
                // Build processedChildren entry (similar to success flow structure)
                processedChildren.push({
                  userId: existingUser.id,
                  childId: familyChild.id,
                  childName: familyChild.child_name,
                  familyId: familyChild.family_id,
                  subscriptionId: userSubscriptions.length > 0 ? userSubscriptions[0].id : null, // Use first active subscription
                  userEmail: existingUser.email,
                  userFullName: existingUser.full_name
                });
                
                // Collect subscription IDs (similar to success flow)
                userSubscriptions.forEach(sub => {
                  foundSubscriptionIds.push(sub.id);
                });
                
                console.log(`[FAILED PAYMENT] Processed child ${familyChild.id}: user ${existingUser.id}, ${userSubscriptions.length} subscriptions`);
              } else {
                console.warn(`[FAILED PAYMENT] User not found for child: ${familyChild.child_name} (ID: ${familyChild.id})`);
              }
            } catch (childError) {
              console.error(`[FAILED PAYMENT] Error processing child ${familyChild.id}:`, childError);
            }
          }
          
          console.log(`[FAILED PAYMENT] Processed ${processedChildren.length} children with user/subscription info`);
        } catch (childLookupError) {
          console.warn(`[FAILED PAYMENT] Error looking up child/student IDs:`, childLookupError);
        }
      }
    }
    
    // Extract student_ids and subscription_ids from processedChildren (same pattern as success flow)
    const studentIds = processedChildren.map(c => c.userId);
    const subscriptionIds = [...new Set(foundSubscriptionIds)]; // Remove duplicates

    // Create family payment transaction record for failed payment
    const familyTransactionData = {
      payment_link_id: familyPaymentLink.id,
      transaction_token: `fam_failed_${transaction_uid}`,
      payplus_transaction_id: transaction_uid,
      family_id: selectedChildren[0]?.familyId || null,
      paid_children_ids: paidChildrenIds,
      paid_children_details: processedChildren.length > 0 ? processedChildren.map(pc => ({
        childId: pc.childId,
        childName: pc.childName,
        familyId: pc.familyId,
        userId: pc.userId,
        subscriptionId: pc.subscriptionId
      })) : selectedChildren, // Use processedChildren if available, fallback to selectedChildren
      student_ids: studentIds, // Extract from processedChildren (same as success flow: processedChildren.map(c => c.userId))
      subscription_ids: subscriptionIds, // From foundSubscriptionIds (same as success flow: updatedSubscriptions)
      amount: amount,
      currency: currency_code || 'ILS',
      payment_type: familyPaymentLink.payment_type,
      status: 'failed',
      payment_method: payment_method || 'unknown',
      card_last_digits: four_digits ? four_digits.slice(-4) : null,
      error_code: status_code,
      error_message: messageDescription,
      payplus_response_data: JSON.stringify(originalWebhook),
      processed_at: new Date()
    };

    const familyPaymentTransaction = await FamilyPaymentTransaction.create(familyTransactionData, { transaction });
    console.log(`Created family failed payment transaction: ${familyPaymentTransaction.id} with ${paidChildrenIds.length} children, ${studentIds.length} students, and ${subscriptionIds.length} subscriptions`);

    // Keep payment link active for retry (don't mark as used)
    // Only update if not already expired or cancelled
    if (familyPaymentLink.status === 'active') {
      console.log('Payment link kept active for retry');
      // Don't update status - keep it active so they can retry
    }

    // Process each family for status updates and activity logging
    const processedFamilies = new Set();
    for (const childDetail of selectedChildren) {
      if (!childDetail.childId) {
        console.warn(`Skipping child without ID for failed payment:`, childDetail);
        continue;
      }

      // Add family to processed set and log failed payment activity
      if (childDetail.familyId && !processedFamilies.has(childDetail.familyId)) {
        processedFamilies.add(childDetail.familyId);

        // Log family activity for failed payment
        await FamilyActivityLog.create({
          family_id: childDetail.familyId,
          user_id: familyPaymentLink.sales_user_id || 1,
          action_type: 'payment_completed', // Keep same type but with failure details
          action_description: `Family payment FAILED. Transaction: ${transaction_uid}. Error: ${status_code}. Payment link remains active for retry.`,
          new_values: {
            transaction_id: transaction_uid,
            amount: amount,
            currency: currency_code,
            payment_type: familyPaymentLink.payment_type,
            children_count: selectedChildren.filter(c => c.familyId === childDetail.familyId).length,
            payment_status: 'failed',
            error_code: status_code,
            retry_available: true
          },
          metadata: {
            payplus_transaction_id: transaction_uid,
            payment_link_id: familyPaymentLink.id,
            family_payment_transaction_id: familyPaymentTransaction.id,
            failure_details: {
              status_code: status_code,
              error_message: familyTransactionData.error_message,
              payment_method: payment_method,
              card_last_digits: four_digits ? four_digits.slice(-4) : null
            }
          }
        }, { transaction });

        console.log(`Logged failed payment activity for family ${childDetail.familyId}`);
      }
    }

    // Send failure notification emails to families (don't wait for completion)
    const emailPromises = [];
    for (const familyId of processedFamilies) {
      try {
        const family = await Family.findByPk(familyId, {
          include: [{
            model: FamilyChild,
            as: 'children',
            where: { id: { [Op.in]: selectedChildren.map(c => c.childId) } },
            required: false
          }],
          transaction
        });

        if (family && family.parent_email) {
          const familyChildren = selectedChildren.filter(c => c.familyId === familyId);
          
          emailPromises.push(
            sendFamilyPaymentFailureEmail(
              family.parent_email,
              family.parent_name,
              familyChildren,
              {
                transaction_id: transaction_uid,
                amount: familyChildren.reduce((sum, c) => sum + parseFloat(c.amount || 0), 0),
                currency: currency_code,
                payment_type: familyPaymentLink.payment_type,
                error_code: status_code,
                retry_link_token: linkToken
              }
            )
          );
        }
      } catch (emailError) {
        console.error(`Error preparing failure email for family ${familyId}:`, emailError.message);
      }
    }

    // Send failure emails (don't wait for completion to avoid blocking)
    Promise.all(emailPromises).then(() => {
      console.log(`Sent failure notification emails to ${emailPromises.length} families`);
    }).catch(error => {
      console.error('Error sending failure notification emails:', error);
    });

    // Handle recurring payment failures - DUNNING SYSTEM INTEGRATION
    if (is_recurring && selectedChildren.length > 0) {
      console.log(`[DUNNING] Detected failed recurring family payment for ${selectedChildren.length} children`);
      try {
        // Get the primary family ID (use the first child's family)
        const primaryFamilyId = selectedChildren[0]?.familyId;
        
        // Extract student_ids and subscription_ids from the transaction we just created
        let transactionStudentIds = [];
        let transactionSubscriptionIds = [];
        if (familyPaymentTransaction.student_ids) {
          try {
            const parsed = typeof familyPaymentTransaction.student_ids === 'string' 
              ? JSON.parse(familyPaymentTransaction.student_ids) 
              : familyPaymentTransaction.student_ids;
            if (Array.isArray(parsed)) {
              transactionStudentIds = parsed.map(id => Number(id));
            }
          } catch (parseError) {
            console.warn(`[DUNNING] Error parsing student_ids from transaction:`, parseError);
          }
        }
        if (familyPaymentTransaction.subscription_ids) {
          try {
            const parsed = typeof familyPaymentTransaction.subscription_ids === 'string'
              ? JSON.parse(familyPaymentTransaction.subscription_ids)
              : familyPaymentTransaction.subscription_ids;
            if (Array.isArray(parsed)) {
              transactionSubscriptionIds = parsed.map(id => Number(id));
            }
          } catch (parseError) {
            console.warn(`[DUNNING] Error parsing subscription_ids from transaction:`, parseError);
          }
        }
        
        if (primaryFamilyId) {
          await handleFamilyRecurringPaymentFailure({
            family_id: primaryFamilyId,
            transaction_uid: transaction_uid,
            amount: parseFloat(amount || 0),
            currency: currency_code || 'ILS',
            failed_at: new Date(),
            status_code: status_code,
            status_description: messageDescription,
            recurring_info: recurring_info,
            page_request_uid: webhookData.page_request_uid,
            webhookData: webhookData.original_webhook || webhookData,
            family_payment_transaction_id: familyPaymentTransaction.id,
            selected_children: selectedChildren,
            family_payment_link_id: familyPaymentLink.id,
            student_ids: transactionStudentIds.length > 0 ? transactionStudentIds : studentIds, // Use from transaction or extracted
            subscription_ids: transactionSubscriptionIds.length > 0 ? transactionSubscriptionIds : subscriptionIds // Use from transaction or extracted
          }, transaction);

          console.log(`[DUNNING] Dunning process initiated for family ${primaryFamilyId}`);
        } else {
          console.warn(`[DUNNING] No family ID found for recurring payment failure - skipping dunning`);
        }
      } catch (dunningError) {
        console.error(`[DUNNING] Error initiating dunning process for family payment:`, dunningError);
      }
    }

    // Final verification logging
    paymentLogger.logPaymentVerification({
      student_id: linkToken,
      student_name: customer_name || 'Family Failed Payment',
      subscription_id: familyPaymentTransaction.id,
      verification_type: 'family_failed_payment_processing_complete',
      verification_result: true, // Processing completed successfully even though payment failed
      subscription_details: {
        family_payment_transaction_id: familyPaymentTransaction.id,
        families_processed: processedFamilies.size,
        children_processed: selectedChildren.length,
        total_amount: amount,
        currency: currency_code,
        is_recurring: is_recurring,
        payment_link_kept_active: true,
        failure_code: status_code
      },
      error_details: {
        payment_failure_code: status_code,
        payment_failure_message: familyTransactionData.error_message,
        payment_method: payment_method,
        card_last_digits: four_digits ? four_digits.slice(-4) : null
      }
    });

    console.log(`Successfully processed family failed payment: ${transaction_uid}`);

    // Log summary
    console.log(`Family failed payment processing summary:`, {
      transactionId: transaction_uid,
      linkToken: linkToken,
      familiesProcessed: processedFamilies.size,
      childrenProcessed: selectedChildren.length,
      totalAmount: amount,
      currency: currency_code,
      errorCode: status_code,
      isRecurring: is_recurring,
      failedPaymentTransactionId: familyPaymentTransaction.id,
      paymentLinkKeptActive: true,
      retryAvailable: true
    });

  } catch (error) {
    console.error('Error in processFamilyPaymentFailedWebhook:', error);
    
    // Log processing error
    paymentLogger.logPaymentVerification({
      student_id: webhookData.more_info_1 || 'unknown',
      student_name: webhookData.customer_name || 'unknown',
      subscription_id: null,
      verification_type: 'family_failed_payment_processing_error',
      verification_result: false,
      error_details: {
        error_type: 'family_failed_processing_exception',
        error_message: error.message,
        error_stack: error.stack,
        transaction_uid: webhookData.transaction_uid,
        webhook_data_available: !!webhookData
      }
    });
    
    throw error;
  }
};

/**
 * Send family payment failure notification email
 * @param {String} parentEmail - Parent email address
 * @param {String} parentName - Parent name
 * @param {Array} children - Array of children details
 * @param {Object} failureDetails - Failure transaction details
 */
const sendFamilyPaymentFailureEmail = async (parentEmail, parentName, children, failureDetails) => {
  try {
    const emailParams = {
      parent_name: parentName,
      children_count: children.length,
      children_names: children.map(c => c.childName).join(', '),
      attempted_amount: failureDetails.amount,
      currency: failureDetails.currency,
      transaction_id: failureDetails.transaction_id,
      error_code: failureDetails.error_code,
      retry_url: `${process.env.FRONTEND_URL}/family-payment/${failureDetails.retry_link_token}`,
      failure_date: new Date().toLocaleDateString(),
      support_email: process.env.SUPPORT_EMAIL || 'support@tulkka.com'
    };

    const emailResult = await sendCombinedNotifications(
      'family_payment_failure', // You'll need this email template
      emailParams,
      {
        email: parentEmail,
        full_name: parentName,
        language: 'EN'
      },
      false
    );

    if (emailResult.emailSent) {
      console.log(`Family payment failure email sent to ${parentEmail}`);
    } else {
      console.error(`Failed to send family payment failure email to ${parentEmail}`);
    }

    return emailResult;
  } catch (error) {
    console.error('Error sending family payment failure email:', error);
    return { emailSent: false, error: error.message };
  }
};

/**
 * Handle recurring family payment failure when PayPlus does NOT send a link token.
 * We derive family/children from the items array and create the failed transaction
 * + past due + dunning, similar to the normal flow.
 */
const processFamilyRecurringFailureWithoutLink = async (webhookData, transaction) => {
  const {
    transaction_uid,
    amount,
    currency_code,
    customer_name,
    customer_email,
    payment_method,
    four_digits,
    status_code,
    recurring_info,
    original_webhook
  } = webhookData;

  console.log(`[DUNNING] Processing recurring family failure without link token for transaction ${transaction_uid}`);

  // Get items from nested webhook (preferred) or original_webhook fallback
  const items =
    original_webhook?.data?.items ||
    webhookData.data?.items ||
    [];

  if (!items || items.length === 0) {
    throw new Error('No items found in recurring family failure webhook - cannot resolve family/children');
  }

  const familyIds = new Set();
  const childIds = new Set();

  for (const item of items) {
    const name = item.name || '';
    // Expect names like "... (FID 25) - (CID 107)"
    const match = name.match(/\(FID\s+(\d+)\)\s*-\s*\(CID\s+(\d+)\)/);
    if (match) {
      familyIds.add(parseInt(match[1], 10));
      childIds.add(parseInt(match[2], 10));
    } else {
      console.warn('[DUNNING] Could not parse FID/CID from item name:', name);
    }
  }

  if (familyIds.size === 0 || childIds.size === 0) {
    throw new Error('Unable to extract family/child IDs from recurring failure items');
  }

  const primaryFamilyId = Array.from(familyIds)[0];
  const childIdsArray = Array.from(childIds);
  console.log(`[DUNNING] Recurring failure parsed family ${primaryFamilyId}, child IDs [${childIdsArray.join(', ')}]`);

  // Look up FamilyChild records and build processedChildren structure (similar to success flow)
  let paidChildrenIds = [];
  const processedChildren = []; // Similar to success flow - track processed children with user/subscription info
  const foundSubscriptionIds = []; // Similar to success flow - track subscription IDs
  let selectedChildren = [];
  
  if (childIdsArray.length > 0) {
    try {
      const familyChildren = await FamilyChild.findAll({
        where: {
          id: { [Op.in]: childIdsArray },
          family_id: primaryFamilyId
        },
        attributes: ['id', 'child_name', 'child_email', 'family_id'],
        include: [{
          model: Family,
          as: 'family',
          attributes: ['parent_email'],
          required: false
        }],
        transaction
      });
      
      paidChildrenIds = familyChildren.map(fc => fc.id);
      
      // Process each child individually (same pattern as success flow)
      for (const familyChild of familyChildren) {
        try {
          let existingUser = null;
          
          // Look up User by child email (same pattern as success flow)
          if (familyChild.child_email) {
            existingUser = await User.findOne({
              where: { email: familyChild.child_email },
              attributes: ['id', 'email', 'full_name'],
              transaction
            });
            
            if (existingUser) {
              console.log(`[DUNNING] Found user by child email: ${existingUser.email} for child ${familyChild.id}`);
            }
          }
          
          // Fallback: try generated email pattern (same as success flow)
          if (!existingUser && familyChild.family?.parent_email) {
            const emailParts = familyChild.family.parent_email.split('@');
            if (emailParts.length === 2) {
              const generatedEmail = `${emailParts[0]}+familychild${familyChild.id}@${emailParts[1]}`;
              existingUser = await User.findOne({
                where: { email: generatedEmail },
                attributes: ['id', 'email', 'full_name'],
                transaction
              });
              
              if (existingUser) {
                console.log(`[DUNNING] Found user by generated email pattern: ${existingUser.email}`);
              }
            }
          }
          
          // Fallback: try by name and account type (same as success flow)
          if (!existingUser) {
            existingUser = await User.findOne({
              where: { 
                full_name: familyChild.child_name,
                account_type: 'family_child'
              },
              attributes: ['id', 'email', 'full_name'],
              transaction
            });
            
            if (existingUser) {
              console.log(`[DUNNING] Found user by name and account type: ${existingUser.full_name}`);
            }
          }
          
          if (existingUser) {
            // Find subscriptions for this user (similar to success flow)
            const userSubscriptions = await UserSubscriptionDetails.findAll({
              where: {
                user_id: existingUser.id,
                status: { [Op.ne]: 'inactive' } // Only active subscriptions
              },
              attributes: ['id', 'user_id'],
              transaction
            });
            
            // Get item for this child
            const item = items.find(it => (it.name || '').includes(`(CID ${familyChild.id})`)) || items[0];
            
            // Build processedChildren entry (similar to success flow structure)
            processedChildren.push({
              userId: existingUser.id,
              childId: familyChild.id,
              childName: familyChild.child_name,
              familyId: familyChild.family_id,
              subscriptionId: userSubscriptions.length > 0 ? userSubscriptions[0].id : null, // Use first active subscription
              amount: item?.amount_pay || amount || 0,
              userEmail: existingUser.email,
              userFullName: existingUser.full_name
            });
            
            // Collect subscription IDs (similar to success flow)
            userSubscriptions.forEach(sub => {
              foundSubscriptionIds.push(sub.id);
            });
            
            console.log(`[DUNNING] Processed child ${familyChild.id}: user ${existingUser.id}, ${userSubscriptions.length} subscriptions`);
          } else {
            console.warn(`[DUNNING] User not found for child: ${familyChild.child_name} (ID: ${familyChild.id})`);
            
            // Fallback: build selectedChildren entry without user info
            const item = items.find(it => (it.name || '').includes(`(CID ${familyChild.id})`)) || items[0];
            selectedChildren.push({
              childId: familyChild.id,
              familyId: primaryFamilyId,
              childName: familyChild.child_name || (item?.name || '').split('(')[0].trim(),
              amount: item?.amount_pay || amount || 0
            });
          }
        } catch (childError) {
          console.error(`[DUNNING] Error processing child ${familyChild.id}:`, childError);
          
          // Fallback: build selectedChildren entry
          const item = items.find(it => (it.name || '').includes(`(CID ${familyChild.id})`)) || items[0];
          selectedChildren.push({
            childId: familyChild.id,
            familyId: primaryFamilyId,
            childName: familyChild.child_name || (item?.name || '').split('(')[0].trim(),
            amount: item?.amount_pay || amount || 0
          });
        }
      }
      
      // Use processedChildren for selectedChildren if available, otherwise use fallback
      if (processedChildren.length > 0) {
        selectedChildren = processedChildren.map(pc => ({
          childId: pc.childId,
          familyId: pc.familyId,
          childName: pc.childName,
          amount: pc.amount,
          studentId: pc.userId
        }));
      }
      
      console.log(`[DUNNING] Extracted ${paidChildrenIds.length} child IDs, processed ${processedChildren.length} children with user/subscription info`);
    } catch (childLookupError) {
      console.warn(`[DUNNING] Error looking up FamilyChild records:`, childLookupError);
      // Fallback: build selectedChildren from item names only
      selectedChildren = childIdsArray.map(childId => {
        const item = items.find(it => (it.name || '').includes(`(CID ${childId})`)) || items[0];
        return {
          childId,
          familyId: primaryFamilyId,
          childName: (item?.name || '').split('(')[0].trim(),
          amount: item?.amount_pay || amount || 0
        };
      });
      paidChildrenIds = childIdsArray;
    }
  }
  
  // Extract student_ids and subscription_ids from processedChildren (same pattern as success flow)
  const studentIds = processedChildren.map(c => c.userId);
  const subscriptionIds = [...new Set(foundSubscriptionIds)]; // Remove duplicates

  // Extract message_description from webhook (same as PayPlus sends)
  let messageDescription = `Family recurring payment failed with status: ${status_code}`;
  if (original_webhook?.transaction?.message_description) {
    messageDescription = original_webhook.transaction.message_description;
  } else if (webhookData?.transaction?.message_description) {
    messageDescription = webhookData.transaction.message_description;
  } else if (original_webhook?.message_description) {
    messageDescription = original_webhook.message_description;
  } else if (webhookData?.message_description) {
    messageDescription = webhookData.message_description;
  }

  // Create minimal family payment transaction for this failed recurring charge
  const familyTransactionData = {
    payment_link_id: null,
    transaction_token: `fam_failed_${transaction_uid}`,
    payplus_transaction_id: transaction_uid,
    family_id: primaryFamilyId,
    paid_children_ids: paidChildrenIds,
    paid_children_details: processedChildren.length > 0 ? processedChildren.map(pc => ({
      childId: pc.childId,
      childName: pc.childName,
      familyId: pc.familyId,
      userId: pc.userId,
      subscriptionId: pc.subscriptionId,
      amount: pc.amount
    })) : selectedChildren, // Use processedChildren if available, fallback to selectedChildren
    student_ids: studentIds, // Extract from processedChildren (same as success flow: processedChildren.map(c => c.userId))
    subscription_ids: subscriptionIds, // From foundSubscriptionIds (same as success flow: updatedSubscriptions)
    amount: amount,
    currency: currency_code || 'ILS',
    payment_type: 'recurring',
    status: 'failed',
    payment_method: payment_method || 'unknown',
    card_last_digits: four_digits ? four_digits.slice(-4) : null,
    error_code: status_code,
    error_message: messageDescription,
    payplus_response_data: JSON.stringify(original_webhook || webhookData),
    processed_at: new Date()
  };

  const familyPaymentTransaction = await FamilyPaymentTransaction.create(familyTransactionData, { transaction });
  console.log(`[DUNNING] Created family failed recurring transaction: ${familyPaymentTransaction.id} with ${paidChildrenIds.length} children, ${studentIds.length} students, and ${subscriptionIds.length} subscriptions`);

  // Trigger the standard family dunning handler
  await handleFamilyRecurringPaymentFailure({
    family_id: primaryFamilyId,
    transaction_uid,
    amount: parseFloat(amount || 0),
    currency: currency_code || 'ILS',
    failed_at: new Date(),
    status_code,
    status_description: messageDescription,
    recurring_info,
    page_request_uid: webhookData.page_request_uid,
    webhookData: original_webhook || webhookData,
    family_payment_transaction_id: familyPaymentTransaction.id,
    selected_children: selectedChildren,
    family_payment_link_id: null,
    student_ids: studentIds, // Pass extracted student_ids
    subscription_ids: subscriptionIds // Pass extracted subscription_ids
  }, transaction);

  console.log(`[DUNNING] Completed recurring family failure processing without link token for family ${primaryFamilyId}`);
};

/**
 * Handle family payment failed page callback - GET endpoint (if needed)
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
const handleFamilyPaymentFailedPage = async (req, res) => {
  try {
    const { token } = req.query;

    if (!token) {
      return res.status(400).json({
        status: 'error',
        message: 'Payment token is required'
      });
    }

    // Verify the family payment link exists and get details
    const familyPaymentLink = await FamilyPaymentLink.findOne({
      where: { link_token: token },
      include: [{
        model: FamilyPaymentTransaction,
        as: 'transactions',
        where: { status: 'failed' },
        required: false,
        limit: 5,
        order: [['created_at', 'DESC']]
      }]
    });

    if (!familyPaymentLink) {
      return res.status(404).json({
        status: 'error',
        message: 'Family payment link not found'
      });
    }

    const hasFailedPayments = familyPaymentLink.transactions && familyPaymentLink.transactions.length > 0;
    
    return res.status(200).json({
      status: 'success',
      data: {
        payment_link: familyPaymentLink,
        has_failed_payments: hasFailedPayments,
        failed_transactions: hasFailedPayments ? familyPaymentLink.transactions : [],
        children_count: familyPaymentLink.selected_children_details?.length || 0,
        total_amount: familyPaymentLink.total_amount,
        currency: familyPaymentLink.currency,
        payment_type: familyPaymentLink.payment_type,
        retry_available: familyPaymentLink.status === 'active'
      },
      message: 'Family payment failure details retrieved successfully'
    });

  } catch (error) {
    console.error('Error handling family payment failed page:', error);
    return res.status(500).json({
      status: 'error',
      message: 'Internal server error',
      details: error.message
    });
  }
};

/**
 * Handle family recurring payment failure - create past due record and start dunning process
 * Similar to handleRecurringPaymentFailure but for family payments
 */
const handleFamilyRecurringPaymentFailure = async (failureData, transaction) => {
    try {
        const {
            family_id,
            transaction_uid,
            amount,
            currency,
            failed_at,
            status_code,
            status_description,
            recurring_info,
            page_request_uid,
            webhookData,
            family_payment_transaction_id,
            selected_children,
            family_payment_link_id,
            student_ids: providedStudentIds,
            subscription_ids: providedSubscriptionIds
        } = failureData;

        console.log(`[DUNNING] Starting dunning process for failed family recurring payment - Family: ${family_id}, Amount: ${amount}`);

        // Extract error details from webhook data
        let failureStatusCode = status_code || '';
        let failureMessageDescription = status_description || '';

        if (webhookData?.original_webhook) {
            const originalWebhook = webhookData.original_webhook;
            if (originalWebhook.transaction) {
                failureStatusCode = originalWebhook.transaction.status_code || failureStatusCode;
                failureMessageDescription = originalWebhook.transaction.message_description || failureMessageDescription;
            }
            failureStatusCode = originalWebhook.status_code || failureStatusCode;
            failureMessageDescription = originalWebhook.message_description || failureMessageDescription;
        }

        if (webhookData && (!failureStatusCode || !failureMessageDescription)) {
            failureStatusCode = webhookData.status_code || failureStatusCode;
            failureMessageDescription = webhookData.message_description || failureMessageDescription;
        }

        console.log(`[DUNNING] Extracted error details - Code: ${failureStatusCode}, Description: ${failureMessageDescription}`);

        // Get family details
        const family = await Family.findByPk(family_id, {
            attributes: ['id', 'parent_name', 'parent_email', 'parent_phone', 'parent_country_code', 'status'],
            transaction
        });

        if (!family) {
            throw new Error(`Family ${family_id} not found for recurring payment failure`);
        }

        // Get recurring payment UID from the family payment transaction or recurring_info
        let recurringPaymentUid = null;
        // Prefer normalized recurring_info object created in family-payment-callback.controller
        if (recurring_info) {
            recurringPaymentUid = recurring_info.recurring_payment_uid
                || recurring_info.recurring_uid
                || null;
        }
        // Fallbacks from raw webhook if not found above
        if (!recurringPaymentUid && webhookData?.recurring_payment_uid) {
            recurringPaymentUid = webhookData.recurring_payment_uid;
        } else if (!recurringPaymentUid && webhookData?.original_webhook?.recurring_payment_uid) {
            recurringPaymentUid = webhookData.original_webhook.recurring_payment_uid;
        }

        // Extract student IDs and subscription IDs from provided data, selected_children, or look them up
        let studentIds = (providedStudentIds && Array.isArray(providedStudentIds)) ? [...providedStudentIds] : [];
        let subscriptionIds = (providedSubscriptionIds && Array.isArray(providedSubscriptionIds)) ? [...providedSubscriptionIds] : [];

        // Try to get from selected_children first - check multiple possible property names
        if (selected_children && Array.isArray(selected_children) && selected_children.length > 0) {
            selected_children.forEach(child => {
                // Try multiple possible property names for student ID
                const studentId = child.studentId || child.student_id || child.userId || child.user_id || null;
                if (studentId) {
                    studentIds.push(Number(studentId));
                }
                // Try multiple possible property names for subscription ID
                const subscriptionId = child.subscriptionId || child.subscription_id || null;
                if (subscriptionId) {
                    subscriptionIds.push(Number(subscriptionId));
                }
            });
            console.log(`[DUNNING] Extracted ${studentIds.length} student IDs from selected_children`);
        }

        // If not found in selected_children, try to get from family payment transaction
        if (studentIds.length === 0 && family_payment_transaction_id) {
            const familyTransaction = await FamilyPaymentTransaction.findByPk(family_payment_transaction_id, {
                attributes: ['student_ids', 'subscription_ids'],
                transaction
            });
            
            if (familyTransaction) {
                try {
                    if (familyTransaction.student_ids) {
                        const parsed = typeof familyTransaction.student_ids === 'string' 
                            ? JSON.parse(familyTransaction.student_ids) 
                            : familyTransaction.student_ids;
                        if (Array.isArray(parsed)) {
                            studentIds = parsed.map(id => Number(id));
                        }
                    }
                    if (familyTransaction.subscription_ids) {
                        const parsed = typeof familyTransaction.subscription_ids === 'string'
                            ? JSON.parse(familyTransaction.subscription_ids)
                            : familyTransaction.subscription_ids;
                        if (Array.isArray(parsed)) {
                            subscriptionIds = parsed.map(id => Number(id));
                        }
                    }
                    console.log(`[DUNNING] Extracted ${studentIds.length} student IDs and ${subscriptionIds.length} subscription IDs from family payment transaction`);
                } catch (parseError) {
                    console.warn(`[DUNNING] Error parsing student/subscription IDs from transaction:`, parseError);
                }
            }
        }

        // If still not found, try to get from FamilyChild records by matching child emails to User records
        if (studentIds.length === 0 && selected_children && Array.isArray(selected_children) && selected_children.length > 0) {
            try {
                const childIds = selected_children
                    .map(child => child.childId || child.child_id || child.id)
                    .filter(id => id != null);
                
                if (childIds.length > 0) {
                    const familyChildren = await FamilyChild.findAll({
                        where: {
                            id: { [Op.in]: childIds },
                            family_id: family_id
                        },
                        attributes: ['id', 'child_email'],
                        transaction
                    });
                    
                    // Look up User records by matching child email
                    const childEmails = familyChildren
                        .map(fc => fc.child_email)
                        .filter(email => email && email.trim() !== '');
                    
                    if (childEmails.length > 0) {
                        const users = await User.findAll({
                            where: {
                                email: { [Op.in]: childEmails }
                            },
                            attributes: ['id', 'email'],
                            transaction
                        });
                        
                        const foundStudentIds = users.map(u => Number(u.id));
                        if (foundStudentIds.length > 0) {
                            studentIds = foundStudentIds;
                            console.log(`[DUNNING] Extracted ${studentIds.length} student IDs by matching child emails to User records`);
                            
                            // Look up subscription_ids from UserSubscriptionDetails for these users
                            try {
                                const subscriptions = await UserSubscriptionDetails.findAll({
                                    where: {
                                        user_id: { [Op.in]: studentIds },
                                        status: { [Op.ne]: 'inactive' } // Only active subscriptions
                                    },
                                    attributes: ['id', 'user_id'],
                                    transaction
                                });
                                
                                const foundSubscriptionIds = subscriptions.map(s => Number(s.id));
                                if (foundSubscriptionIds.length > 0) {
                                    subscriptionIds = [...subscriptionIds, ...foundSubscriptionIds];
                                    console.log(`[DUNNING] Found ${foundSubscriptionIds.length} subscription IDs for ${studentIds.length} students`);
                                }
                            } catch (subError) {
                                console.warn(`[DUNNING] Error looking up subscription IDs:`, subError);
                            }
                        }
                    }
                }
            } catch (childLookupError) {
                console.warn(`[DUNNING] Error looking up student/subscription IDs from FamilyChild/User:`, childLookupError);
            }
        }

        // Ensure we have valid arrays (remove duplicates and nulls)
        studentIds = [...new Set(studentIds.filter(id => id != null && !isNaN(id)))];
        subscriptionIds = [...new Set(subscriptionIds.filter(id => id != null && !isNaN(id)))];

        console.log(`[DUNNING] Final extracted ${studentIds.length} student IDs and ${subscriptionIds.length} subscription IDs`);
        if (studentIds.length === 0) {
            console.warn(`[DUNNING] WARNING: No student IDs found for family ${family_id} - past due payment will be created without student_ids`);
        }
        if (subscriptionIds.length === 0) {
            console.warn(`[DUNNING] WARNING: No subscription IDs found for family ${family_id} - past due payment will be created without subscription_ids`);
        }

        // Check for existing past due payment for this family
        let pastDuePayment = await FamilyPastDuePayment.findOne({
            where: {
                family_id: family_id,
                status: 'past_due'
            },
            transaction
        });

        const gracePeriodDays = parseInt(process.env.DUNNING_GRACE_PERIOD_DAYS) || 30;
        const gracePeriodExpiry = moment(failed_at).add(gracePeriodDays, 'days').toDate();
        const isNewRecord = !pastDuePayment;

        if (pastDuePayment) {
            // Update existing past due payment with new attempt and error details
            const updatedNotes = `${pastDuePayment.notes || ''}\n[${new Date().toISOString()}] Additional failure: ${failureMessageDescription || status_description} (Code: ${failureStatusCode || status_code}). Transaction: ${transaction_uid}`;
            
            // Prepare update data - include student_ids and subscription_ids if we have them
            const updateData = {
                amount: amount,
                currency: currency,
                attempt_number: pastDuePayment.attempt_number + 1,
                failure_status_code: failureStatusCode,
                failure_message_description: failureMessageDescription,
                notes: updatedNotes
            };
            
            // Always update student_ids and subscription_ids - FamilyPastDuePayment uses TEXT type, so we need JSON.stringify
            updateData.student_ids = JSON.stringify(studentIds || []);
            updateData.subscription_ids = JSON.stringify(subscriptionIds || []);
            
            await pastDuePayment.update(updateData, { transaction });

            console.log(`[DUNNING] Updated existing family past due payment ${pastDuePayment.id} - attempt #${pastDuePayment.attempt_number + 1}`);
            
            await pastDuePayment.reload({ transaction });
        } else {
            // Create new family past due payment record with error details
            const initialNotes = `Initial failure: ${failureMessageDescription || status_description} (Code: ${failureStatusCode || status_code}). Transaction: ${transaction_uid}`;
            
            pastDuePayment = await FamilyPastDuePayment.create({
                family_id: family_id,
                family_payment_transaction_id: family_payment_transaction_id,
                recurring_payment_uid: recurringPaymentUid,
                amount: amount,
                currency: currency,
                failed_at: failed_at,
                due_date: moment(failed_at).format('YYYY-MM-DD'),
                grace_period_days: gracePeriodDays,
                grace_period_expires_at: gracePeriodExpiry,
                status: 'past_due',
                attempt_number: 1,
                payplus_page_request_uid: page_request_uid,
                failure_status_code: failureStatusCode,
                failure_message_description: failureMessageDescription,
                children_count: selected_children.length,
                student_ids: JSON.stringify(studentIds || []), // FamilyPastDuePayment uses TEXT type, so we need JSON.stringify
                subscription_ids: JSON.stringify(subscriptionIds || []), // FamilyPastDuePayment uses TEXT type, so we need JSON.stringify
                notes: initialNotes
            }, { transaction });

            console.log(`[DUNNING] Created new family past due payment record ${pastDuePayment.id} with error details`);
        }

        // Auto-cancel recurring using same day-based rule as individual flow:
        // first failure after last success older than 29 days.
        // Keep 30-failed-attempts as fallback for legacy behavior.
        const currentAttemptNumber = pastDuePayment.attempt_number;
        let shouldAutoCancel = false;
        let autoCancelReason = null;

        try {
            const lastSuccess = await FamilyPaymentTransaction.findOne({
                where: {
                    family_id,
                    payment_type: 'recurring',
                    status: 'success'
                },
                attributes: ['created_at'],
                order: [['created_at', 'DESC']],
                transaction
            });

            const failWhere = {
                family_id,
                payment_type: 'recurring',
                status: 'failed'
            };
            if (lastSuccess?.created_at) {
                failWhere.created_at = { [Op.gt]: lastSuccess.created_at };
            }

            const firstFail = await FamilyPaymentTransaction.findOne({
                where: failWhere,
                attributes: ['created_at'],
                order: [['created_at', 'ASC']],
                transaction
            });

            if (firstFail?.created_at) {
                const firstFailedAt = new Date(firstFail.created_at);
                const now = new Date();
                const msPerDay = 24 * 3600 * 1000;
                const daysDiff = Math.floor((now - firstFailedAt) / msPerDay);

                console.log(`[AUTO-CANCEL][FAMILY] Family ${family_id} first failed at ${firstFailedAt.toISOString()} (${daysDiff} days ago)`);

                if (daysDiff > 29) {
                    shouldAutoCancel = true;
                    autoCancelReason = `Auto-cancel: first failed at ${firstFailedAt.toISOString()} (${daysDiff} days since)`;
                }
            }
        } catch (daysCheckError) {
            console.error(`[AUTO-CANCEL][FAMILY] Error while checking day-based auto-cancel for family ${family_id}:`, daysCheckError);
        }

        if (!shouldAutoCancel && currentAttemptNumber >= 30) {
            shouldAutoCancel = true;
            autoCancelReason = `Auto-cancel after ${currentAttemptNumber} failed recurring attempts`;
        }

        if (shouldAutoCancel) {
            try {
                // Cancel family recurring payment at PayPlus
                if (recurringPaymentUid) {
                    await cancelFamilyRecurringPayment(recurringPaymentUid);
                    console.log(`[DUNNING] Auto-cancelled family recurring payment at PayPlus: ${recurringPaymentUid}`);
                }

                // Update all RecurringPayment records for this family's children
                if (studentIds.length > 0) {
                    await RecurringPayment.update(
                        { status: 'cancelled', is_active: false },
                        {
                            where: {
                                student_id: { [Op.in]: studentIds },
                                is_active: true
                            },
                            transaction
                        }
                    );
                    console.log(`[DUNNING] Auto-cancelled recurring payments for ${studentIds.length} students`);

                    // Inactivate active subscriptions for all affected children
                    await UserSubscriptionDetails.update(
                        {
                            status: 'inactive',
                            is_cancel: 1,
                            updated_at: new Date()
                        },
                        {
                            where: {
                                user_id: { [Op.in]: studentIds },
                                status: 'active',
                                is_cancel: 0
                            },
                            transaction
                        }
                    );

                    // Clear user-level subscription pointers
                    await User.update(
                        {
                            subscription_id: null,
                            subscription_type: null
                        },
                        {
                            where: { id: { [Op.in]: studentIds } },
                            transaction
                        }
                    );

                    console.log(`[DUNNING] Inactivated subscriptions and cleared subscription pointers for ${studentIds.length} students`);
                }

                // Update past due payment status
                await pastDuePayment.update({
                    status: 'canceled',
                    canceled_at: new Date(),
                    cancellation_reason: autoCancelReason
                }, { transaction });

                console.log(`[DUNNING] Auto-cancelled family recurring payments for family ${family_id}. Reason: ${autoCancelReason}`);
            } catch (autoCancelError) {
                console.error(`[DUNNING] Error auto-cancelling family recurring payments:`, autoCancelError);
            }
        }

        // Set recovery page URL with short_id (like regular past_due_payments)
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
        let shortId = pastDuePayment.short_id;
        if (!shortId) {
            shortId = generateShortId();
            await pastDuePayment.update({
                short_id: shortId
            }, { transaction });
        }
        const recoveryPageUrl = `${frontendUrl}/family-payment/recovery/${shortId}`;
        
        await pastDuePayment.update({
            payment_link: recoveryPageUrl,
            payplus_page_request_uid: null
        }, { transaction });

        console.log(`[DUNNING] Recovery page URL saved: ${recoveryPageUrl}`);

        // Create or update family dunning schedule
        let dunningSchedule = await FamilyDunningSchedule.findOne({
            where: { 
                family_past_due_payment_id: pastDuePayment.id 
            },
            transaction
        });

        const familyTimezone = 'Asia/Jerusalem'; // Default timezone, can be enhanced later
        const reminderTime = process.env.DUNNING_REMINDER_TIME || '10:00';
        const [hours, minutes] = reminderTime.split(':').map(Number);
        const nextReminderAt = moment().tz(familyTimezone).add(1, 'day').hour(hours).minute(minutes).second(0).toDate();

        if (dunningSchedule) {
            // Update existing schedule - clear any pause and reset for new failure
            await dunningSchedule.update({
                is_enabled: true,
                is_paused: false,
                paused_until: null, // Clear any previous pause
                next_reminder_at: nextReminderAt,
                timezone: familyTimezone
            }, { transaction });
        } else {
            // Create new family dunning schedule
            dunningSchedule = await FamilyDunningSchedule.create({
                family_past_due_payment_id: pastDuePayment.id,
                family_id: family_id,
                is_enabled: true,
                is_paused: false,
                reminder_frequency: 'daily',
                reminder_time: `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:00`,
                timezone: familyTimezone,
                next_reminder_at: nextReminderAt
            }, { transaction });

            console.log(`[DUNNING] Created family dunning schedule ${dunningSchedule.id} - next reminder at ${nextReminderAt}`);
        }

        // Calculate actual days remaining based on preserved grace_period_expires_at
        const actualGracePeriodExpiry = pastDuePayment.grace_period_expires_at || gracePeriodExpiry;
        const daysRemaining = Math.max(0, Math.ceil(moment(actualGracePeriodExpiry).diff(moment(), 'days', true)));

        // Send immediate failure notification (Day 0)
        // TODO: Implement family payment failure notification similar to regular payments
        console.log(`[DUNNING] Immediate failure notification should be sent to family ${family_id} (${daysRemaining} days remaining)`);

        // Log the dunning process initiation with error details
        paymentLogger.logPaymentVerification({
            student_id: `family_${family_id}`,
            student_name: family.parent_name || 'Family',
            subscription_id: family_payment_transaction_id,
            verification_type: 'family_dunning_process_initiated',
            verification_result: true,
            subscription_details: {
                family_past_due_payment_id: pastDuePayment.id,
                family_id: family_id,
                grace_period_days: gracePeriodDays,
                grace_period_expires_at: actualGracePeriodExpiry,
                recovery_link_generated: !!recoveryPageUrl,
                children_count: selected_children.length,
                failure_status_code: failureStatusCode,
                failure_message_description: failureMessageDescription
            }
        });

        console.log(`[DUNNING] Family dunning process initiated successfully for family ${family_id} with error details: ${failureStatusCode} - ${failureMessageDescription}`);

    } catch (error) {
        console.error('[DUNNING] Error in handleFamilyRecurringPaymentFailure:', error);
        
        paymentLogger.logPaymentVerification({
            student_id: failureData.family_id ? `family_${failureData.family_id}` : 'unknown',
            student_name: 'unknown',
            subscription_id: null,
            verification_type: 'family_dunning_process_error',
            verification_result: false,
            error_details: {
                error_type: 'family_dunning_initiation_error',
                error_message: error.message,
                error_stack: error.stack,
                failure_data: failureData
            }
        });
        
        throw error;
    }
};

/**
 * Wrapper function for callback controller - matches expected signature
 * @param {Object} webhookData - Extracted PayPlus webhook data
 * @param {Number} webhookLogId - ID of the webhook log entry
 * @param {Object} familyMetadata - Family metadata (optional)
 * @param {Object} transaction - Database transaction
 */
const processFamilyFailedWebhookPayment = async (webhookData, webhookLogId, familyMetadata, transaction) => {
  // Call the main processing function
  return await processFamilyPaymentFailedWebhook(webhookData, webhookLogId, transaction);
};

module.exports = {
  processFamilyPaymentFailed,
  processFamilyPaymentFailedWebhook,
  processFamilyFailedWebhookPayment, // Wrapper for callback controller
  sendFamilyPaymentFailureEmail,
  handleFamilyPaymentFailedPage,
  handleFamilyRecurringPaymentFailure
};