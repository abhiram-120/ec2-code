...
const { Family, FamilyPaymentTransaction, FamilyChild, FamilyActivityLog, FamilyCartItem } = require('../../models/Family');
...
/** Map duration months to cart subscription_type enum */
const durationMonthsToCartSubscriptionType = (months) => {
    const m = parseInt(months, 10);
    if (!m || Number.isNaN(m)) return 'monthly';
  ...
  try {
    ...
    const childrenDetails = selectedChildrenWithSubscriptions.map((child) => {
      const normalizedPlanType =
        (child.planType || child.plan_type || '').toString().toLowerCase() === 'custom'
          ? 'custom'
          : 'default';
      ...
      return {
        ...
        // Keep both keys for compatibility across older/newer success handlers.
        planType: normalizedPlanType,
        plan_type: normalizedPlanType,
        pricing_type: normalizedPlanType,
    ...
    if (paymentType === 'recurring') {
      ...
      payPlusRequest.recurring_settings = {
        ...
        // recurring_type: 0, // Daily 1 for weekly, 2 for monthly
    ...
    // Persist Default / Custom (and cart subscription snapshot) on cart rows for this salesperson
    const salesUserIdForCart = req.user?.id;
    if (salesUserIdForCart) {
      try {
        for (const child of selectedChildrenWithSubscriptions) {
          const dm = parseInt(child.durationMonths || child.customMonths || 1, 10);
          const subType = durationMonthsToCartSubscriptionType(dm);
          const ptRaw = (child.planType || 'default').toString().toLowerCase();
          const isCustom = ptRaw === 'custom';
          const amt = parseFloat(Number(child.amount).toFixed(2));
          await FamilyCartItem.update(
            ...
            {
              where: {
                sales_user_id: salesUserIdForCart,
        ...
        console.error(
          '[generateFamilyPaymentLink] Failed to sync cart_pricing_type:',
          cartSyncErr
        );
      }
    }
    ...
    // ---------- Log family activity: payment link generated ----------
    try {
      const salesUserId = req.user?.id || 1;
      // Activity log: omit internal DB / PayPlus IDs and raw id arrays (admin UI + exports).
      const childrenSummaryForActivityLog = childrenDetails.map((c) => ({
        childName: c.childName,
        relationshipToParent: c.relationshipToParent,
      ...
      const activityMetadata = {
        ...
        payplus_payment_url: paymentUrl,
        payplus_page_request_uid: pageRequestUid,
        children_summary: childrenSummaryForActivityLog,
      };
      if (shortUrl) {
        activityMetadata.payplus_short_url = shortUrl;
      }
      if (hostedFieldsUuid) {
        activityMetadata.payplus_hosted_fields_uuid = hostedFieldsUuid;
      }
      for (const familyId of familyIds) {
        await FamilyActivityLog.create({
          family_id: familyId,
          user_id: salesUserId,
          action_type: 'payment_generated',
          action_description: `Family payment link generated. ${childrenCount} children, total ${Number(totalAmount).toFixed(2)} ${currency}.`,
          new_values: {
            total_amount: Number(totalAmount),
            currency,
            payment_type: paymentType,
      ...
      console.error('Error logging payment_generated activity for families:', activityError);
    }
...
const sendFamilyPaymentLinkWhatsApp = async (req, res) => {
    try {
        ...
        // Build a human-readable summary of each child's plan (single-line string for WhatsApp template param)
        let childrenSummary = '';
        if (family_details && Array.isArray(family_details.children) && family_details.children.length > 0) {
            childrenSummary = family_details.children
                .map((child, index) => {
                    const rawChildName =
                        child.childName ||
                        child.child_name ||
                    ...
                    const childName = String(rawChildName).replace(/[\r\n]+/g, ' ').trim();

                    const lessonsPerMonth =
                        child.lessonsPerMonth ||
                        child.lessons_per_month ||
                        child.lessons_per_months ||
                        child.lessonsPerMonthDefault ||
                        child.lessonsPerMonthSelected ||
                        child.lessonsPerMonthConfigured ||
                        child.lessonsPerMonthCount;

                    const lessonMinutes =
                        child.lessonMinutes ||
                        child.lesson_min ||
                        child.lessonDuration ||
                        child.lesson_minutes;

                    const lessonsText = lessonsPerMonth
                        ? `${lessonsPerMonth} שיעורים בחודש`
                        : 'מספר שיעורים בחודש';

                    const minutesText = lessonMinutes
                        ? `${lessonMinutes} דקות לשיעור`
                        : 'משך שיעור בדקות';

                    return `ילד ${index + 1}: ${childName} – ${lessonsText} / ${minutesText}`;
                })
                // Single-line param to satisfy WhatsApp API restriction
                .join(' | ');
        }

        // Prepare WhatsApp template parameters for family payment
        // These keys should match the variables defined in the Aisensy template
        const whatsappParams = {
            'parent.name': parent_name,
            'family.childrenSummary': childrenSummary,
            'payment.link': short_payment_link
        };
