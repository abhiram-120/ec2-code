...
const getPaymentTransactions = async (req, res) => {
    try {
        ...
        const conditions = {};
        const andConditions = [];
        const normalizedPaymentMethod = Sequelize.fn(
            'LOWER',
            Sequelize.fn('COALESCE', Sequelize.col('PaymentTransaction.payment_method'), '')
        );
    ...
    [Op.between]: [
      ...
      new Date(`${end_date} 23:59:59`)
        ...
        if (payment_type === 'online') {
            andConditions.push(
                Sequelize.where(normalizedPaymentMethod, 'unknown')
            );
        }

        if (payment_type === 'offline') {
            andConditions.push(
                Sequelize.where(normalizedPaymentMethod, { [Op.ne]: 'unknown' })
            );
        }
