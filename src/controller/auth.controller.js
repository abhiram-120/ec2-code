...
async function updateProfile(req, res) {
    try {
        ...
        // Enhanced email validation that allows child formatted emails:
        // baseemail+alias with spaces@domain.com
        ...
        if (value.email !== undefined && value.email !== null && value.email !== '') {
            ...
            const normalizedReqEmail = value.email.trim().toLowerCase();
            const normalizedCurrentEmail = (user.email || '').trim().toLowerCase();
            if (normalizedReqEmail !== normalizedCurrentEmail) {
                const existingEmailUser = await Users.findOne({
                    where: {
                        email: value.email,
                        id: { [Op.ne]: user.id }
        ...
        // Detect if caller explicitly changed email/mobile
        const emailWasExplicitlyChanged =
            value.email !== undefined &&
            value.email !== null &&
            value.email !== '' &&
            (value.email.trim().toLowerCase() !== (user.email || '').trim().toLowerCase());
        const mobileWasExplicitlyChanged =
            value.mobile !== undefined &&
            value.mobile !== null &&
            value.mobile !== '' &&
            value.mobile !== user.mobile;

        let finalEmail = value.email !== undefined && value.email !== null && value.email !== '' ? value.email : user.email;
        let finalMobile = value.mobile !== undefined && value.mobile !== null && value.mobile !== '' ? value.mobile : user.mobile;

        if (value.full_name !== user.full_name) {
            if (user.guardian) {
                const emailBase = (user.email || '').replace(/\+[^@]+/, '');
                const mobileBase = (user.mobile || '').replace(/\+[^+]+$/, '');
                const cleanName = (value.full_name || '').trim().replace(/[^a-zA-Z0-9\s]/g, '');
                const nameWithSpaces = cleanName.replace(/\s+/g, ' ').trim();

                if (!emailWasExplicitlyChanged && emailBase && nameWithSpaces) {
                    const kidEmailAt = emailBase.indexOf('@');
                    if (kidEmailAt > -1) {
                        finalEmail = emailBase.slice(0, kidEmailAt) + '+' + nameWithSpaces + emailBase.slice(kidEmailAt);
                    }
                }

                if (!mobileWasExplicitlyChanged && mobileBase && nameWithSpaces) {
                    finalMobile = mobileBase + '+' + nameWithSpaces;
                    if (finalMobile.length > 32) {
                        const maxNameLen = 32 - mobileBase.length - 1;
                        finalMobile = mobileBase + '+' + nameWithSpaces.substring(0, Math.max(0, maxNameLen));
                    }
                }
            }
        }
        ...
        // Update PayPlus customer details (name/email/phone) if online payment history exists
        ...
        try {
            let latestPayment = await PaymentTransaction.findOne({
                where: {
                    [Op.or]: [
                        ...
                        { student_email: updatedUserData.email }
                    ],
                    status: 'success',
            ...
            // Fallback: try finding by customer_email inside response_data
            if (!latestPayment) {
                const candidateEmails = [...new Set([user.email, updatedUserData.email, value.email].filter(Boolean).map((e) => e.toLowerCase()))];
                if (candidateEmails.length > 0) {
                    ...
                    for (const payment of allPayments) {
                        const responseObj = parseResponseData(payment.response_data);
                        const customerEmail = responseObj?.customer_email || responseObj?.data?.customer_email || null;
                        if (customerEmail && candidateEmails.includes(String(customerEmail).toLowerCase())) {
            ...
            if (latestPayment?.response_data) {
                let responseData = parseResponseData(latestPayment.response_data);
                ...
                if (customerUid) {
                    ...
                    if (value.full_name !== undefined) {
                        updatePayload.customer_name = updatedUserData.full_name;
                        oldValues.full_name = user.full_name;
                        newValues.full_name = updatedUserData.full_name;
                    }

                    if (value.email !== undefined) {
                        let payplusEmail = updatedUserData.email;
                        if (payplusEmail && payplusEmail.includes('+')) {
                            ...
                            if (emailParts.length === 2) {
                                ...
                                if (baseEmail && studentNamePart) {
                                    const hyphenAlias = studentNamePart.replace(/\s+/g, '-');
                                    payplusEmail = `${baseEmail}+${hyphenAlias}@${domain}`;
                                }
                            }
                        }
                    ...
                    if (value.mobile !== undefined) {
                        let payplusPhone = updatedUserData.mobile;
                        if (payplusPhone && payplusPhone.includes('+')) {
                            const parts = payplusPhone.split('+');
                            if (parts.length >= 2) {
                                const basePhone = parts[0];
                                const alias = parts[1];
                                if (basePhone && alias) {
                                    const hyphenAlias = alias.replace(/\s+/g, '-');
                                    payplusPhone = `${basePhone}+${hyphenAlias}`;
                                    if (payplusPhone.length > 20) {
                                        const maxAliasLen = 20 - basePhone.length - 1;
                                        payplusPhone = `${basePhone}+${hyphenAlias.substring(0, Math.max(0, maxAliasLen))}`;
                                    }
                                }
                            }
                        }
                    ...
                    if (Object.keys(updatePayload).length > 0) {
                        ...
                        payplusUpdateLogger.logPayPlusUpdateSkipped({
                            student_id: user.id,
                            student_email: updatedUserData.email,
                            reason: 'No profile fields changed for PayPlus'
                        });
                    }
                } else {
                    payplusUpdateLogger.logPayPlusUpdateSkipped({
            ...
            console.error('[updateProfile][PayPlus] update error', {
                userId: user.id,
                error: payplusError.message
            });
        ...
        // In-app notification: profile/settings updated successfully
        try {
            await whatsappReminderAddClass('settings_updated_success', {}, user.id);
        } catch (notifyErr) {
            console.error('Error sending settings_updated_success notification:', notifyErr);
        }
        ...
        res.status(200).json({
            ...
            data: {
                ...
                ...(payplusUpdateResult ? { payplus_update: payplusUpdateResult } : {})
            }
        });
    } catch (err) {
        // In-app notification: profile/settings update failed
        try {
            await whatsappReminderAddClass('settings_update_failed', {}, req.userId);
        } catch (notifyErr) {
            console.error('Error sending settings_update_failed notification:', notifyErr);
        }
...
async function loginStudent(req, res) {
    try {
        ...
        // Track login + activity timestamps (unix seconds)
        const now = Math.floor(Date.now() / 1000);
        await Users.update({ last_login_at: now, last_active_at: now }, { where: { id: user.id } });
...
async function testFCMNotification(req, res) {
    try {
        ...
        const result = await firebaseService.sendNotificationToDevice(
            ...
            { title, body },
            data,
            { userId, userName, template, channel: 'inapp', context }
        );
