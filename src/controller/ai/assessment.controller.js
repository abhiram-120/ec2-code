...
const startAssessment = async (req, res) => {
    ...
    try {
        ...
        const skillFocuses = ['vocabulary', 'grammar', 'listening'];
        const limit = parseLimit(req.body?.limit, 12);
        const perLevel = Math.max(skillFocuses.length, Math.floor(limit / levels.length));
        const effectiveLimit = Math.max(limit, perLevel * levels.length);

        const questions = [];
        const usedIds = new Set();

        for (const level of levels) {
            // Ensure each level contains each skill_focus (if available).
            for (const skill of skillFocuses) {
                let picked = await AssessmentQuestion.findAll({
                    where: {
                ...
                if (!picked.length) {
                    picked = await AssessmentQuestion.findAll({
                        where: {
                            difficulty_level: level,
                            id: { [Op.notIn]: Array.from(usedIds) },
                            disabled: { [Op.ne]: true }
            ...
            const extraNeeded = Math.max(perLevel - skillFocuses.length, 0);
            if (extraNeeded > 0) {
                const extras = await AssessmentQuestion.findAll({
                    where: {
                        difficulty_level: level,
                        id: { [Op.notIn]: Array.from(usedIds) },
                        disabled: { [Op.ne]: true }
        ...
        if (questions.length < effectiveLimit) {
            const remaining = effectiveLimit - questions.length;
...
const submitAssessment = async (req, res) => {
    ...
    try {
        ...
        const skillFocuses = ['vocabulary', 'grammar', 'listening'];
        const skillStats = skillFocuses.reduce((acc, skill) => {
        ...
        const gradedAnswers = answers.map(a => {
            ...
            if (question?.skill_focus && skillStats[question.skill_focus]) {
                skillStats[question.skill_focus].total += 1;
                if (isCorrect) skillStats[question.skill_focus].correct += 1;
            }
            return {
        ...
        const skillAccuracy = skillFocuses.map(skill => {
            const stats = skillStats[skill];
            const accuracy = stats.total > 0
                ? Number(((stats.correct / stats.total) * 100).toFixed(2))
                : null;
            return { skill_focus: skill, total: stats.total, correct: stats.correct, accuracy };
        });
        ...
        return res.status(200).json({
            ...
            result: {
                ...
                skill_focus_breakdown: skillAccuracy
            }
        });
    } catch (error) {
        await transaction.rollback();
