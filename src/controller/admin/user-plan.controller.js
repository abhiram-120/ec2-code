...
const getUserPlans = async (req, res) => {
    try {
        ...
        const filterDateColumn = status === 'inactive' ? 'cancellation_date' : 'created_at';
        ...
        if (status === 'inactive') {
            ...
            subscriptionWhereConditions.cancellation_reason_category_id = {
                [Op.ne]: null
            };
        ...
        if (date_from && date_to) {
            subscriptionWhereConditions[filterDateColumn] = {
        ...
        if (date_from && date_to) {
            subscriptionWhereSQL += ` AND usd.${filterDateColumn} BETWEEN :date_from AND :date_to`;
        ...
        const rankedSubscriptionsBaseSQL = status === 'inactive'
            ? `
                SELECT
                    usd.id,
                    ROW_NUMBER() OVER (
                        PARTITION BY usd.user_id
                        ORDER BY COALESCE(usd.cancellation_date, usd.updated_at, usd.created_at) DESC, usd.id DESC
                    ) as rn
                ...
                AND usd.cancellation_reason_category_id IS NOT NULL
                AND NOT EXISTS (
                    SELECT 1
                    FROM user_subscription_details active_usd
                    WHERE active_usd.user_id = usd.user_id
                    AND active_usd.status = 'active'
                )
            `
            : `
                SELECT
        ...
        const [teachers, transactionsByEmail, familyTxByChildName, firstSubscriptionsMap, priceCache] = await Promise.all([
            ...
            // Batch fetch all payment transactions grouped by email / user_id
            (async () => {
                ...
                if (userEmails.length > 0) {
                    const allTransactions = await PaymentTransaction.findAll({
                        ...
                        attributes: ['id', 'student_id', 'student_email', 'amount', 'refund_amount', 'status', 'is_recurring', 'payment_method', 'card_last_digits', 'created_at', 'transaction_id'],
                        order: [['created_at', 'DESC']],
                        raw: true
                    });
                    ...
                    allTransactions.forEach((tx) => {
                        ...
                        // Also index by student_id so we can match even if email differs
                        if (tx.student_id) {
                            const idKey = `id:${tx.student_id}`;
                            if (!transactionsMap.has(idKey)) {
                                transactionsMap.set(idKey, []);
                            }
                            transactionsMap.get(idKey).push(tx);
                        }
                    });
                }
            ...
            // Batch fetch family payment transactions grouped by child name
            (async () => {
                const familyMap = new Map();

                const allFamilyTx = await FamilyPaymentTransaction.findAll({
                    where: { status: 'success' },
                    attributes: ['id', 'paid_children_details', 'amount', 'currency', 'payment_method', 'created_at'],
                    raw: true
                });

                allFamilyTx.forEach((ft) => {
                    ...
                    childDetails.forEach((c) => {
                        ...
                        if (!shareAmount || isNaN(shareAmount)) return;
                        ...
                        if (!familyMap.has(childName)) {
                            familyMap.set(childName, []);
                        }
                        familyMap.get(childName).push(entry);
                    });
                });
        ...
        const formatted = latestPlans.map((subscription) => {
            ...
            try {
                ...
                const studentIdForTx = subData.user_id || subData.SubscriptionUser?.id;
                const studentNameForFamily = subData.SubscriptionUser?.full_name;

                if (userEmail || studentIdForTx || studentNameForFamily) {
                    const byEmail = userEmail ? (transactionsByEmail.get(userEmail) || []) : [];
                    const byId = studentIdForTx ? (transactionsByEmail.get(`id:${studentIdForTx}`) || []) : [];
                    const byFamilyName = studentNameForFamily
                        ? (familyTxByChildName.get(studentNameForFamily) || [])
                        : [];

                    // Merge and de-duplicate transactions (by id or transaction_id)
                    const mergedList = [...byEmail, ...byId, ...byFamilyName];
                    const seen = new Set();
                    const allTransactions = [];
                    mergedList.forEach((tx) => {
                        const key = tx.id || tx.transaction_id || `${tx.student_email}_${tx.amount}_${tx.created_at}`;
                        if (!seen.has(key)) {
                            seen.add(key);
...
const exportUserPlansCSV = async (req, res) => {
    try {
        ...
        // ----------------------------------------
        //  SAME FILTERS AS getUserPlans (subscriptionWhereConditions + dates)
        // ----------------------------------------
        const subscriptionWhere = {};

        if (status === 'inactive') {
            subscriptionWhere.status = 'inactive';
            subscriptionWhere.cancellation_reason_category_id = {
                [Op.ne]: null
            };
        } else if (status === 'inactive_after_renew') {
            ...
            subscriptionWhere.status = 'active';
            subscriptionWhere.inactive_after_renew = 0;
        }
        ...
        if (date_from && date_to) {
            subscriptionWhere[filterDateColumn] = {
                [Op.between]: [
                    moment.tz(date_from, 'YYYY-MM-DD', 'UTC').startOf('day').toDate(),
                    moment.tz(date_to, 'YYYY-MM-DD', 'UTC').endOf('day').toDate()
                ]
            };
        }
        ...
        rawData.forEach(sub => {
            if (sub.cancelled_by_user_id) teacherIds.add(sub.cancelled_by_user_id);
        });
        ...
        const formatted = rawData.map((sub) => {
            ...
            const cancelledByUser =
                d.cancelled_by_user_id
                    ? teachersMap.get(d.cancelled_by_user_id)
                    : null;
            let currentBonusReason = '';
            let bonusClassHistory = [];
            ...
            const plan = {
                ...
                cancelled_by_user_name: cancelledByUser?.full_name || null,
                cancelled_by_user_email: cancelledByUser?.email || null,
                cancellation_reason_category: d.CancellationReasonCategory ? d.CancellationReasonCategory.name : null,
        ...
        filtered.forEach((p) => {
            ...
            rows.push([
                ...
                p.cancellation_date
                    ...
                    : "N/A",
                p.cancellation_reason_category || "N/A",
                p.cancellation_reason || "N/A",
                p.cancelled_by_user_name || "N/A",
        ...
        await ActionLog.create(
            {
                ...
                action_type: 'create subscription',
                target_entity: 'subscription',
                target_id: user_id,
                old_value: null,
                new_value: {
                    subscription_type: type,
                    monthly_lessons: parseInt(weekly_lesson),
                    status,
                    payment_status,
                    offline_payment_reason: offline_payment_reason.trim()
                },
...
const updateUserPlan = async (req, res) => {
    ...
    try {
        ...
        const oldActionLogValue = {
            subscription_type: subscription.type,
            monthly_lessons: subscription.weekly_lesson,
            status: subscription.status,
            payment_status: subscription.payment_status,
            offline_payment_reason: subscription.offline_payment_reason
        };
        ...
        const isSubscriptionInactivation = ['inactive', 'inactive_after_renew'].includes(status);

        await ActionLog.create(
            {
                actor_id: req.user.id,
                action_type: isSubscriptionInactivation ? 'inactivate subscription' : 'update subscription',
                target_entity: 'subscription',
                target_id: subscription.user_id,
                old_value: isSubscriptionInactivation ? null : oldActionLogValue,
                new_value: isSubscriptionInactivation
                    ? {
                        subscription_type: type ?? subscription.type,
                        monthly_lessons: weekly_lesson !== undefined ? parseInt(weekly_lesson) : subscription.weekly_lesson,
                        status,
                        payment_status: payment_status ?? subscription.payment_status,
                        offline_payment_reason:
                            payment_status === 'offline'
                                ? offline_payment_reason.trim()
                                : subscription.offline_payment_reason,
                        cancellation_reason: cancellation_reason ?? null,
                        cancellation_type: cancellation_type ?? null
                    }
                    : {
                        subscription_type: type ?? subscription.type,
                        monthly_lessons: weekly_lesson !== undefined ? parseInt(weekly_lesson) : subscription.weekly_lesson,
...
const downloadUserPlanInvoice = async (req, res) => {
    try {
        ...
        let transactionUid = paymentTransaction
            ? (paymentTransaction.transaction_id || paymentTransaction.token)
            : null;

        // Fallback for family payments: try to find matching FamilyPaymentTransaction
        if (!transactionUid) {
            const student = userPlan.SubscriptionUser;
            const studentName = student?.full_name;

            if (studentName) {
                const familyTransactions = await FamilyPaymentTransaction.findAll({
                    where: { status: 'success' },
                    attributes: ['payplus_transaction_id', 'paid_children_details', 'created_at'],
                    order: [['created_at', 'DESC']],
                    raw: true
                });
                ...
                for (const ft of familyTransactions) {
                    ...
                    const match = childDetails.find((c) => {
                        const childName = c.childName || c.child_name;
                        return childName === studentName;
                    });

                    if (match && ft.payplus_transaction_id) {
                        transactionUid = ft.payplus_transaction_id;
                        break;
                    }
                }
            }
