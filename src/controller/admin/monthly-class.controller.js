        ...
        await ActionLog.create(
            {
                ...
                action_type: 'book class',
                target_entity: 'class',
                target_id: bookedClasses.length > 0 ? bookedClasses[0].id : null,
                old_value: null,
                new_value: {
                    student_id,
                    teacher_id,
                    ...
                    batch_id: batchId,
                    booking_type: validationResult.bookingType,
                    booked_class_ids: bookedClasses.map((cls) => cls.id),
                    regular_class_ids: regularClassEntries.map((entry) => entry.id),
...
const deleteRegularClass = async (req, res) => {
    ...
    try {
        ...
        const oldRegularClassValue = JSON.parse(JSON.stringify(regularClass.get({ plain: true })));
        ...
        await ActionLog.create(
            {
                actor_id: req.user.id,
                action_type: 'delete regular class',
                target_entity: 'class',
                target_id: parseInt(id),
                old_value: oldRegularClassValue,
                new_value: {
                    deleted_regular_class_id: parseInt(id),
                    batch_id: regularClass.batch_id,
                    cancel_future_classes,
