...
const createManualEvent = async (req, res) => {
    try {
        const { student_id, event_type, display_name, description, points, impact_level, valid_until } = req.body;
        const created_by = req.user.role_name;
        const normalizedDisplayName = display_name || description;

        if (!student_id || !event_type || !normalizedDisplayName || !points || !impact_level || !valid_until) {
