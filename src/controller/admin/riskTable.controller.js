...
async function getFreshNextClassDateMap(studentIds) {
    ...
    const nextClassDateMap = new Map();

    upcomingClasses.forEach((lesson) => {
        if (!nextClassDateMap.has(lesson.student_id)) {
            nextClassDateMap.set(lesson.student_id, lesson.meeting_start);
        }
    });

    return nextClassDateMap;
}
...
const buildRiskTableWhere = (params = {}) => {
    const {
        risk_level,
        payment_method,
        recurring_lessons,
    ...
    const contactStatuses = normalizeArrayFilter(contact_status);
    if (contactStatuses.length) {
        where.contact_status = { [Op.in]: contactStatuses };
    }

    const subscriptionTypes = normalizeArrayFilter(subscription_type);
    if (subscriptionTypes.length) {
        where.subscription_type = { [Op.in]: subscriptionTypes };
    }

    const paymentMethods = normalizeArrayFilter(payment_method);
    ...
    if (recurring_lessons !== undefined && recurring_lessons !== null && recurring_lessons !== '') {
        where.recurring_lessons =
            recurring_lessons === true || recurring_lessons === 'true';
    }
    ...
    if (learningDuration && learningDuration !== 'all') {
        if (String(learningDuration).includes('-')) {
            const [min, max] = String(learningDuration).split('-');
            learning_duration_min = Number(min);
            learning_duration_max = Number(max);
        } else if (String(learningDuration).endsWith('+')) {
            learning_duration_min = Number(String(learningDuration).replace('+', ''));
        }
    }

    if (
        typeof learning_duration_min === 'number' ||
        typeof learning_duration_max === 'number'
    ) {
...
const getRiskTable = async (req, res) => {
    try {
        ...
        const where = buildRiskTableWhere(req.query);
...
========================= */
        ...
        const nextClassDateMap = await getFreshNextClassDateMap(studentIds);
        ...
        const response = risks.map((risk) => {
            ...
            return {
                ...
                next_class_date: nextClassDateMap.get(risk.student_id) || 'N/A',
...
const exportRiskTableCSV = async (req, res) => {
    try {
        ...
        const exportMode = requestData.export_mode || 'all';
        const selectedStudentIds = normalizeArrayFilter(
            requestData.student_ids || requestData.studentIds
        )
            .map((id) => Number(id))
            .filter((id) => !Number.isNaN(id));

        const where =
            exportMode === 'selected' && selectedStudentIds.length
                ? { student_id: { [Op.in]: selectedStudentIds } }
                : buildRiskTableWhere(requestData.filters || requestData);
        ...
        for (const risk of risks) {
            ...
            const assignedRepUser = risk.rep_id
                ? await User.findOne({
                      where: { id: risk.rep_id },
                      attributes: ['full_name']
                  })
                : null;
            const assignedRep = assignedRepUser?.full_name || 'Unassigned';
