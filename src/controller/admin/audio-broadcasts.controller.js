...
const createAudioBroadcast = async (req, res) => {
    try {
        ...
        // Validate audio file
        if (!req.files || (!req.files.audioFile && !req.files.audio)) {
            return res.status(400).json({
                status: 'error',
        ...
        // Upload audio file to S3
        const audioFile = (req.files.audioFile && req.files.audioFile[0]) || (req.files.audio && req.files.audio[0]);
        ...
        const coverImageFile = (req.files && req.files.imageFile && req.files.imageFile[0]) ||
...
const updateAudioBroadcast = async (req, res) => {
    try {
        ...
        const incomingAudioFile = (req.files && req.files.audio && req.files.audio[0]) ||
            (req.files && req.files.audioFile && req.files.audioFile[0]);
        if (incomingAudioFile) {
            const audioFile = incomingAudioFile;
            const audioUploadResult = await uploadAudioFile(audioFile, userId);
        ...
        const incomingCoverImage = (req.files && req.files.coverImage && req.files.coverImage[0]) ||
            (req.files && req.files.imageFile && req.files.imageFile[0]);
        if (incomingCoverImage) {
            const coverImage = incomingCoverImage;
            const imageUploadResult = await uploadCoverImage(coverImage, userId);
