...
const { computeNextReminderAt } = require('../../utils/dunningScheduleHelpers');

function isValidReminderFrequency(value) {
    const v = String(value || '').trim();
    if (v === 'daily' || v === 'every_2_days' || v === 'weekly') return true;
    const m = v.match(/^every_(\d{1,3})_days$/);
    if (!m) return false;
...
const getFailedPaymentsList = async (req, res) => {
    try {
        ...
        const now = new Date();
        const effectiveGracePeriodStatus = grace_period_status || (
            days_remaining === '1-7' ? 'expiring_soon'
                : days_remaining === 'expired' ? 'expired'
                : days_remaining === '8-15' || days_remaining === '16-30' ? days_remaining
                : undefined
        );

        if (effectiveGracePeriodStatus === 'expiring_soon') {
            const sevenDaysFromNow = moment().add(7, 'days').toDate();
            whereConditions.grace_period_expires_at = {
                [Op.between]: [now, sevenDaysFromNow]
            };
        } else if (effectiveGracePeriodStatus === '8-15') {
            whereConditions.grace_period_expires_at = {
                [Op.gt]: moment().add(7, 'days').toDate(),
                [Op.lte]: moment().add(15, 'days').toDate()
            };
        } else if (effectiveGracePeriodStatus === '16-30') {
            whereConditions.grace_period_expires_at = {
                [Op.gt]: moment().add(15, 'days').toDate(),
                [Op.lte]: moment().add(30, 'days').toDate()
            };
        } else if (effectiveGracePeriodStatus === 'expired') {
            whereConditions.grace_period_expires_at = { [Op.lte]: now };
        } else if (effectiveGracePeriodStatus === 'normal') {
            const sevenDaysFromNow = moment().add(7, 'days').toDate();
        ...
        if (reminder_status) {
            const dunningWhere = {};
            if (reminder_status === 'active' || reminder_status === 'enabled') {
...
/**
 * Update dunning schedule (admin): reminder_frequency, time, timezone.
 */
const updateDunningSchedule = async (req, res) => {
    try {
        const { id } = req.params;
        const {
            reminder_frequency,
            reminder_time,
            timezone,
            recalculate_next_reminder = true,
        } = req.body || {};
        ...
        if (reminder_frequency !== undefined) {
            if (!isValidReminderFrequency(reminder_frequency)) {
                return res.status(400).json({
                    status: 'error',
                    message: 'reminder_frequency must be daily, every_2_days, weekly, or every_<N>_days (1..90)',
                });
            }
            updates.reminder_frequency = reminder_frequency;
        }
        ...
        await dunningSchedule.update(updates);

        const merged = { ...dunningSchedule.get({ plain: true }), ...updates };
        const userTz =
            merged.timezone ||
            dunningSchedule.PastDuePayment?.User?.timezone ||
            'Asia/Jerusalem';

        if (recalculate_next_reminder && dunningSchedule.is_enabled && !dunningSchedule.is_paused) {
            const nextReminderAt = computeNextReminderAt(merged, userTz);
            await dunningSchedule.update({ next_reminder_at: nextReminderAt });
        }

        await dunningSchedule.reload({
            include: [
                {
                    model: PastDuePayment,
                    as: 'PastDuePayment',
        ...
        paymentLogger.logPaymentVerification({
            ...
            verification_type: 'dunning_schedule_updated_by_admin',
            verification_result: true,
            subscription_details: {
                dunning_schedule_id: dunningSchedule.id,
                updates,
                recalculate_next_reminder,
        ...
        return res.status(200).json({
            status: 'success',
            data: dunningSchedule,
            message: 'Dunning schedule updated successfully',
        });
    } catch (error) {
        console.error('Error updating dunning schedule:', error);
        return res.status(500).json({
            status: 'error',
            message: 'Internal server error',
            details: error.message,
...
const resumeDunningReminders = async (req, res) => {
    try {
        ...
        // Calculate next reminder time from reminder_frequency
        const userTimezone = dunningSchedule.PastDuePayment?.User?.timezone || 'Asia/Jerusalem';
        const schedulePlain = dunningSchedule.get({ plain: true });
        const nextReminderAt = computeNextReminderAt(schedulePlain, userTimezone);
...
const exportFailedPayments = async (req, res) => {
    try {
        ...
        if (date_from || date_to) {
            ...
            if (Object.keys(whereConditions.failed_at).length === 0) {
                delete whereConditions.failed_at;
            }
        }
        ...
        if (days_remaining === '1-7') {
            whereConditions.grace_period_expires_at = {
                [Op.between]: [now, moment().add(7, 'days').toDate()]
            };
...
/**
 * Global dunning: representative `reminder_frequency` from existing schedule rows (PUT bulk-updates all rows).
 */
const getGlobalDunningSettings = async (req, res) => {
    try {
        const row = await DunningSchedule.findOne({
            attributes: ['reminder_frequency'],
            order: [['updated_at', 'DESC']],
        });
        let freq = row?.reminder_frequency;
        if (!freq || !isValidReminderFrequency(freq)) {
            const envFreq = process.env.DUNNING_DEFAULT_REMINDER_FREQUENCY;
            freq = isValidReminderFrequency(envFreq) ? envFreq : 'daily';
        }
...
/**
 * Set `reminder_frequency` on all `dunning_schedules` rows.
 */
const updateGlobalDunningSettings = async (req, res) => {
    try {
        const { reminder_frequency } = req.body || {};

        if (reminder_frequency === undefined || reminder_frequency === '') {
            return res.status(400).json({
                status: 'error',
                message: 'reminder_frequency is required (daily, every_2_days, or weekly)',
            });
        }
        if (!isValidReminderFrequency(reminder_frequency)) {
            return res.status(400).json({
        ...
        await DunningSchedule.update({ reminder_frequency }, { where: {} });
