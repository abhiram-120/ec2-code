...
/**
 * Count successful recurring payments that occur strictly before this transaction
 * (chronological order: created_at, then id). Used so failed/pending retries still
 * show the correct billing-cycle number (e.g. 4th charge attempt after 3 successes).
 * @param {Object} transaction - Current payment row
 * @param {Array<{id: number, created_at: Date}>} sortedSuccessPayments - ASC by date then id
 * @returns {number}
 */
const countPriorSuccessfulRecurringBefore = (transaction, sortedSuccessPayments) => {
    const tt = new Date(transaction.created_at).getTime();
    return sortedSuccessPayments.filter((s) => {
        const ts = new Date(s.created_at).getTime();
        if (ts < tt) return true;
        if (ts > tt) return false;
        return s.id < transaction.id;
    }).length;
};
...
const calculatePaymentSequence = async (transaction, userEmail, planId) => {
    try {
        ...
        const allSuccessful = await PaymentTransaction.findAll({
            where: {
        ...
        return countPriorSuccessfulRecurringBefore(transaction, allSuccessful) + 1;
...
const batchCalculatePaymentSequences = async (transactions) => {
    ...
    try {
        ...
        for (const [key, groupTransactions] of Object.entries(userGroups)) {
            ...
            if (isRecurring === 'true') {
                // Successful payments define prior billing cycles; any row (incl. failed/pending)
                // gets sequence = 1 + count of successes strictly before it in time order.
                const allSuccessful = await PaymentTransaction.findAll({
                    where: {
                        student_email: email,
                        status: 'success',
                ...
                console.log(`Found ${allSuccessful.length} successful recurring payments for ${email}`);

                groupTransactions.forEach((transaction) => {
                    sequenceMap[transaction.id] =
                        countPriorSuccessfulRecurringBefore(transaction, allSuccessful) + 1;
                });
            } else {
...
const processEnhancedRefund = async (req, res) => {
    ...
    try {
        ...
        const oldPaymentValue = payment.get({ plain: true });
        ...
        await ActionLog.create(
            {
                ...
                new_value: {
                    ...
                    refund_type: type,
                    refund_reason: customReason || reason,
                    subscription_action: hasManageableSubscription ? subscriptionAction : null,
                    lessons_deducted: hasManageableSubscription && deductLessonsFromLastRenewal ? lessonsToDeduct : 0,
                    email_notification_sent: emailSent
                },
                created_at: new Date()
            },
