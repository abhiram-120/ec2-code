...
const bulkExportPayslips = async (req, res) => {
  try {
    ...
    const payslips = await TeacherPayslip.findAll({
      ...
      // Keep sorting aligned with getTeacherPayslips pagination order
      // so exported page N matches UI page N.
      order: [
        ['created_at', 'DESC'],
        ['id', 'DESC']
      ]
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="Payslips_${label}_page_${safePage}.zip"`
    );
