...
function createActionLogSnapshot(value) {
...
const createClass = async (req, res) => {
    try {
        ...
        if (user.subscription_id === null) {
            ...
            if (totalClasses > 0) {
                return res.status(400).json({ status: 'error', message: 'Your free trial ended, Please subscribe to your plan...' });
            }
        }
            ...
            await ActionLog.create(
                {
                    ...
                    action_type: 'create class',
                    target_entity: 'class',
                    target_id: newClass.id,
                    old_value: null,
                    new_value: {
                        ...newClass.get({ plain: true }),
                        student_id,
                        teacher_id,
...
const updateClass = async (req, res) => {
    try {
        ...
        const oldClassValue = createActionLogSnapshot(classItem.get({ plain: true }));
        ...
        await ActionLog.create({
            actor_id: req.user.id,
            action_type: 'update class',
            target_entity: 'class',
            target_id: parseInt(id),
            old_value: oldClassValue,
            new_value: createActionLogSnapshot(classItem.get({ plain: true })),
            created_at: new Date()
        });
...
const cancelClass = async (req, res) => {
    ...
    try {
        ...
        const classStart = moment.utc(classItem.meeting_start);
        const minutesUntilClass = classStart.diff(now, 'minutes');
        const paymentStatus = minutesUntilClass >= 0 && minutesUntilClass <= 30 ? 'paid' : 'unpaid';
...
async function checkTeacherAvailabilityForAdmin(teacherId, startDate, duration) {
    ...
    if (!availability) {
        return {
            available: false, // Allow but with warning
