...
const refreshCohortRetention = async (req, res) => {
  try {
    ...
    const effectiveStartMonth = start_month || '2025-08';
    ...
    for (const sub of subRecords) {
      ...
      userIntervals[uid].push({
        ...
        left_lessons: sub.left_lessons ?? null,
     ...
     * -----------------------------------------
     */
    const userAttributes = ['id', 'full_name', 'email', 'mobile', 'country_code', 'trial_user_id'];
    ...
    const paymentIds = [...new Set(
      subRecords
        .map((sub) => sub.payment_id)
        .filter((paymentId) => paymentId !== null && paymentId !== undefined)
    )];

    const paymentMap = {};
    if (paymentIds.length) {
      const paymentRecords = await PaymentTransaction.findAll({
        where: { id: { [Op.in]: paymentIds } },
        attributes: ['id', 'lessons_per_month', 'is_recurring'],
        raw: true
      });

      paymentRecords.forEach((payment) => {
        paymentMap[payment.id] = {
          lessons_per_month: payment.lessons_per_month ?? null,
    ...
    const classRecords = await Class.findAll({
      where: { student_id: userIds },
      attributes: ['student_id', 'teacher_id', 'subscription_id', 'meeting_start', 'created_at'],
      order: [
        ['meeting_start', 'DESC'],
        ['created_at', 'DESC']
      ],
    ...
    const lastClassBySubscription = {};
    const lastClassByUser = {};

    classRecords.forEach((cls) => {
      if (cls.subscription_id && !lastClassBySubscription[cls.subscription_id]) {
        lastClassBySubscription[cls.subscription_id] = cls;
      }

      if (cls.student_id && !lastClassByUser[cls.student_id]) {
        lastClassByUser[cls.student_id] = cls;
      }
    });

    const teacherIds = [...new Set(
      [
        ...Object.values(lastClassBySubscription).map((cls) => cls.teacher_id),
        ...Object.values(lastClassByUser).map((cls) => cls.teacher_id)
      ].filter(Boolean)
    )];
    ...
    if (teacherIds.length) {
      ...
      teacherRecords.forEach((teacher) => {
        teacherNameMap[teacher.id] = teacher.full_name || null;
      });
    }

    const userProfileMap = {};
    users.forEach((u) => {
      const phoneNumber = [u.country_code, u.mobile].filter(Boolean).join(' ').trim() || u.mobile || null;
      userProfileMap[u.id] = {
        id: u.id,
    ...
    const formatPaymentMethod = (paymentStatus) => {
      if (!paymentStatus) return null;
      if (paymentStatus === 'online') return 'Online';
      if (paymentStatus === 'offline') return 'Offline';
    ...
    const getTeacherName = (uid, interval) => {
      const subscriptionClass = interval?.subscription_id
        ? lastClassBySubscription[interval.subscription_id]
        : null;
      const fallbackClass = lastClassByUser[uid];
      const teacherId = subscriptionClass?.teacher_id || fallbackClass?.teacher_id;
      return teacherId ? teacherNameMap[teacherId] || null : null;
    };

    const buildMonthDetailStudent = (uid, interval, monthStart, monthEnd) => {
      const payment = interval?.payment_id ? paymentMap[interval.payment_id] : null;
      const totalClasses = payment?.lessons_per_month ?? null;
      const classesUnused = interval?.left_lessons ?? null;
      const classesUsed =
        totalClasses !== null && classesUnused !== null
          ? Math.max(totalClasses - classesUnused, 0)
          : null;
      ...
      return {
        ...
        subscription_end_date: interval ? formatDate(interval.renew_date || interval.end) : null,
        renew_date: interval ? formatDate(interval.renew_date) : null,
        cancellation_date: interval ? formatDate(interval.cancellation_date) : null,
        cancelled_on: interval ? formatDate(interval.cancellation_date) : null,
        cancellation_reason: interval?.cancellation_reason || null,
        teacher_name: getTeacherName(uid, interval),
        total_classes_in_package: totalClasses,
        classes_used: classesUsed,
        classes_unused: classesUnused,
        payment_method: formatPaymentMethod(interval?.payment_status || null),
        billing_type: payment ? (payment.is_recurring ? 'Recurring' : 'One-time') : null,
    ...
    for (const uid of userIds) {
      ...
      if (effectiveStartMonth && label < effectiveStartMonth) continue;
