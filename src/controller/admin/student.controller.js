...
const { FamilyPaymentTransaction } = require('../../models/Family');
...
async function getStudents(req, res) {
    try {
        ...
        // Inactivity filter (based on last_active_at unix seconds)
        ...
        const [
            ...
            fetchPaymentData(userIds, userNameMap),
...
async function fetchPaymentData(userIds, userNameMap = {}) {
    // Fetch all successful regular payment transactions for totals and dates
    ...
    // Fetch successful FAMILY payment transactions that are linked to any of these students
    // Note: student_ids / paid_children_details are JSON; we will resolve to student IDs in code
    const familyPayments = await FamilyPaymentTransaction.findAll({
        where: { status: 'success' },
        attributes: [
            'id',
            'student_ids',
    ...
    // 2) Family payments – distribute amounts per student
    for (const fp of familyPayments) {
        let detailsRaw = fp.paid_children_details;        
        let childDetails = [];
        if (Array.isArray(detailsRaw)) {
            childDetails = detailsRaw;
        } else if (typeof detailsRaw === 'string') {
            try {
                const parsed = JSON.parse(detailsRaw);
                if (Array.isArray(parsed)) childDetails = parsed;
            } catch {
                // ignore parse error
            }
        }

        // allocation per transaction: uid -> amount
        const allocation = {};

        // Prefer explicit per-child details
        for (const c of childDetails) {
            // 1) Try to map directly via IDs present on the detail
            let uid = Number(
                c.userId ||
                c.studentId ||
                c.student_id
            ...
            // 2) If no valid ID, try to match by childName to student full_name
            if (!uid || isNaN(uid) || !userIdSet.has(uid)) {
                uid = null;
                const childName = c.childName || c.child_name;
                if (childName) {
                    for (const [idStr, name] of Object.entries(userNameMap)) {
                        if (name === childName) {
                            const candidate = Number(idStr);
                            if (!isNaN(candidate) && userIdSet.has(candidate)) {
                                uid = candidate;
                                break;
                            }
                        }
            ...
            const val = Number(c.amount || c.monthlyAmount || 0);
            if (isNaN(val) || !val) continue;

            allocation[uid] = (allocation[uid] || 0) + val;
        }

        // Fallback: if no per-child mapping, but student_ids exist, split equally
        if (Object.keys(allocation).length === 0 && fp.student_ids) {
            let studentIdsRaw = fp.student_ids;
            let studentIds = [];
            if (Array.isArray(studentIdsRaw)) {
                studentIds = studentIdsRaw;
            } else if (typeof studentIdsRaw === 'string') {
                try {
            ...
            if (studentIds && studentIds.length > 0) {
                const split = Number(fp.amount) / studentIds.length;
                const perStudent = isNaN(split) ? 0 : split;
                if (perStudent) {
                    for (const sid of studentIds) {
                        const uid = Number(sid);
                        if (!userIdSet.has(uid)) continue;
                        allocation[uid] = (allocation[uid] || 0) + perStudent;
                    }
                }
            }
        }

        const createdAt = fp.created_at;
        for (const [uidStr, amount] of Object.entries(allocation)) {
            const uid = Number(uidStr);
            if (isNaN(uid) || !amount) continue;

            if (!totalPaidMap[uid]) totalPaidMap[uid] = 0;
            totalPaidMap[uid] += amount;

            if (
                !lastPaymentDateMap[uid] ||
                moment(createdAt).isAfter(lastPaymentDateMap[uid])
            ) {
                lastPaymentDateMap[uid] = createdAt;
                lastPaymentDetailsMap[uid] = {
                    student_id: uid,
                    amount,
                    created_at: createdAt,
                    currency: fp.currency,
                    card_last_digits: fp.card_last_digits,
                    transaction_id: fp.id,
                    payment_method: fp.payment_method || 'family_payment'
                };
            }
        }
    }
...
function buildPaymentInfo(subscription, latestPayment, studentId, paymentData) {
    ...
    if (subscription.payment_status === 'offline') {
        paymentInfo = {
            ...
            amount: actualLatestPayment?.amount || subscription.cost_per_lesson * subscription.weekly_lesson || 0,
            method: 'offline'
        };
    } else if (subscription.payment_status === 'online' && actualLatestPayment) {
        // For online subscriptions, show the subscription's monthly amount (like family list),
        // not the raw transaction total, so that Student list matches Family list pricing.
        let displayAmount = actualLatestPayment.amount;
        if (subscription.type === 'monthly' && subscription.cost_per_lesson && subscription.weekly_lesson) {
            const calculated = Number(subscription.cost_per_lesson) * Number(subscription.weekly_lesson);
            if (!isNaN(calculated) && calculated > 0) {
                displayAmount = calculated;
            }
        }
...
async function addBonusLessons(req, res) {
    ...
    try {
        ...
        const oldSubscriptionValue = subscription.get({ plain: true });
        ...
        const updatedRemainingLessons = (subscription.left_lessons || 0) + parseInt(lessons);
        ...
        await ActionLog.create(
            {
                ...
                action_type: 'add bonus lesson',
                target_entity: 'lesson',
                target_id: parseInt(id),
                old_value: oldSubscriptionValue,
                new_value: {
...
async function returnLessons(req, res) {
  ...
  try {
    ...
    await ActionLog.create(
      {
        actor_id: req.user.id,
        action_type: 'return lesson',
        target_entity: 'lesson',
        target_id: parseInt(id),
        old_value: oldSubscriptionValue,
        new_value: {
          ...subscription.get({ plain: true }),
          returned_lessons: parseInt(lessons),
          reason: reason || null,
          validity_type: validityType,
          days_valid: daysValid || null,
          expires_at: expiryDate
        },
        created_at: new Date()
      },
...
async function expireBonusLessons(req, res) {
    ...
    try {
        ...
        await ActionLog.create(
            {
                actor_id: req.user.id,
                action_type: 'expire bonus lesson',
                target_entity: 'lesson',
                target_id: parseInt(id),
                old_value: oldSubscriptionValue,
                new_value: {
...
async function clearUnusedLessons(req, res) {
    ...
    try {
        ...
        await ActionLog.create(
            {
                actor_id: req.user.id,
                action_type: 'clear unused lesson',
                target_entity: 'lesson',
                target_id: parseInt(id),
                old_value: oldSubscriptionValue,
                new_value: {
...
async function getStudentPaymentHistory(req, res) {
    try {
        ...
        // Get payment transactions with enhanced fields for refunds (all rows; we'll paginate after merging with family payments)
        ...
        const formattedPayments = paymentTransactions.map((payment) => {
            ...
            if (payment.status === 'refunded') {
                const refundProcessor = payment.RefundProcessor;
                const resolvedProcessorName =
                    payment.refund_processed_by_name ||
                    (refundProcessor && refundProcessor.full_name) ||
                    null;
                basePayment.refund_details = {
                    ...
                    refund_processed_by_name: resolvedProcessorName,
        ...
        // ----------------------------------------------
        // ⭐ INCLUDE FAMILY PAYMENTS IN HISTORY
        // ----------------------------------------------

        // Fetch successful family payments within the same optional date range
        const familyWhere = {
            status: { [Op.in]: ['success', 'refunded'] }
        };

        if (from_date) {
            familyWhere.created_at = {
                ...(familyWhere.created_at || {}),
                [Op.gte]: moment(from_date, 'YYYY-MM-DD').startOf('day').toDate()
            };
        }
        ...
        const familyPaymentsForStudent = [];

        for (const ft of familyTransactions) {
            let detailsRaw = ft.paid_children_details;
            let childDetails = [];
            ...
            if (!childDetails.length) continue;

            for (const c of childDetails) {
                const childName = c.childName || c.child_name;
                if (!childName || childName !== student.full_name) continue;

                const shareAmount = Number(c.amount || 0);
                if (!shareAmount || isNaN(shareAmount)) continue;

                const basePayment = {
                    id: `family_${ft.id}_${childName}`,
                    transaction_id: `family_${ft.id}`,
                    type: ft.payment_type === 'recurring' ? 'recurring_family' : 'family',
                    amount: shareAmount,
                    currency: ft.currency || 'ILS',
                    status: ft.status || 'success',
                    payment_method: ft.payment_method || 'family_payment',
                    card_last_digits: ft.card_last_digits,
                    date: ft.created_at,
                    subscription_details: {
                        lessons_per_month: c.lessonsPerMonth || c.lessons_per_month,
                        lesson_minutes: c.lessonMinutes || c.lesson_minutes,
                        custom_months: c.durationMonths || c.duration_months,
                        is_recurring: ft.payment_type === 'recurring'
                    },
                ...
                // Attach simple refund details for refunded family payments
                if ((ft.status === 'refunded') && ft.refund_amount) {
                    const familyRefundProcessor = ft.RefundProcessor;
                    const resolvedFamilyProcessorName =
                        ft.refund_processed_by_name ||
                        (familyRefundProcessor && familyRefundProcessor.full_name) ||
                        null;
                    basePayment.refund_details = {
                        refund_amount: Number(ft.refund_amount) || shareAmount,
                        refund_type: ft.refund_type || 'full',
                        refund_reason: ft.refund_reason || 'Family payment refunded',
                        refund_date: ft.refund_date || ft.created_at,
                        refund_processed_by: ft.refund_processed_by,
                        refund_processed_by_name: resolvedFamilyProcessorName,
                        email_notification_sent: ft.email_notification_sent || false,
                        original_amount: shareAmount,
                        currency: ft.currency || 'ILS',
                        transaction_id: `family_${ft.id}`,
                        student_name: student.full_name,
                        payment_date: ft.created_at
                    };
                }

                // Respect payment_method filter if applied
                if (
                    payment_method &&
                    payment_method !== 'all' &&
                    basePayment.payment_method !== payment_method
                ) {
                    continue;
                }

                familyPaymentsForStudent.push(basePayment);
            }
        }

        // Merge regular + family payments, then sort and paginate
        let allPayments = [...formattedPayments, ...familyPaymentsForStudent];
        ...
        allPayments.sort((a, b) => {
            ...
            return validSortOrder === 'ASC' ? da - db : db - da;
        });
        ...
        // ----------------------------------------------
        // ⭐ UPDATED SUMMARY CALCULATION (INCLUDING FAMILY)
        // ----------------------------------------------
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                pagination: {
                    ...
                    current_page: parseInt(page),
                    total_pages: Math.ceil(totalPaymentsCount / numericLimit),
...
async function getStudentPaymentStats(req, res) {
    try {
        ...
        // Get latest INDIVIDUAL payment (non-family)
        ...
        // ----- Include FAMILY payments for this student's stats -----
        const studentName = student.full_name;

        const familyTransactions = await FamilyPaymentTransaction.findAll({
            where: {
                status: { [Op.in]: ['success', 'refunded'] },
                created_at: dateCondition.created_at
            },
            attributes: [
                'id',
                'paid_children_details',
        ...
        // Aggregate family payments that belong to this student (by childName match)
        let familyTotalPaid = 0;
        let familyPaymentCount = 0;
        let latestFamilyPayment = null;
        const familyMonthlyMap = {}; // month (YYYY-MM) -> aggregates
        ...
        for (const ft of familyTransactions) {
            ...
            for (const c of childDetails) {
                const childName = c.childName || c.child_name;
                if (!childName || childName !== studentName) continue;

                const shareAmount = Number(c.amount || 0);
                if (!shareAmount || isNaN(shareAmount)) continue;

                familyTotalPaid += shareAmount;
                familyPaymentCount += 1;

                const createdAt = ft.created_at;
                const monthKey = moment(createdAt).format('YYYY-MM');

                if (!familyMonthlyMap[monthKey]) {
                    familyMonthlyMap[monthKey] = {
                        total_payments: 0,
                        total_amount: 0,
                        total_refunded: 0,
                ...
                familyMonthlyMap[monthKey].total_payments += 1;
                familyMonthlyMap[monthKey].successful_payments += 1;
                familyMonthlyMap[monthKey].total_amount += shareAmount;

                if (ft.status === 'refunded') {
                    const perChildRefund = ft.refund_amount
                        ? Number(ft.refund_amount) * (shareAmount / Number(ft.amount || shareAmount || 1))
                        : shareAmount;
                    familyMonthlyMap[monthKey].refunded_payments += 1;
                    familyMonthlyMap[monthKey].total_refunded += isNaN(perChildRefund) ? 0 : perChildRefund;
                }

                if (
                    !latestFamilyPayment ||
                    moment(createdAt).isAfter(moment(latestFamilyPayment.created_at))
                ) {
                    latestFamilyPayment = {
                        amount: shareAmount,
        ...
        // Format monthly data (merge individual + family payments)
        const monthlyData = [];
        const paymentStatsMap = {};
        paymentStats.forEach((stat) => {
            paymentStatsMap[stat.month] = stat;
        });
        for (let i = monthsBack - 1; i >= 0; i--) {
            const month = moment().subtract(i, 'months').format('YYYY-MM');
            const monthName = moment().subtract(i, 'months').format('MMM YYYY');

            const monthStat = paymentStatsMap[month] || {};
            const familyMonth = familyMonthlyMap[month] || {
                total_payments: 0,
                total_amount: 0,
                total_refunded: 0,
                successful_payments: 0,
            ...
            const mergedTotalPayments = parseInt(monthStat.total_payments || 0) + familyMonth.total_payments;
            const mergedSuccessful = parseInt(monthStat.successful_payments || 0) + familyMonth.successful_payments;
            const mergedFailed = parseInt(monthStat.failed_payments || 0) + familyMonth.failed_payments;
            const mergedRefunded = parseInt(monthStat.refunded_payments || 0) + familyMonth.refunded_payments;
            const mergedTotalAmount = parseFloat(monthStat.total_amount || 0) + familyMonth.total_amount;
            const mergedTotalRefunded = parseFloat(monthStat.total_refunded || 0) + familyMonth.total_refunded;

            monthlyData.push({
                month: monthName,
                total_payments: mergedTotalPayments,
                total_amount: mergedTotalAmount,
                total_refunded: mergedTotalRefunded,
                net_total: mergedTotalAmount - mergedTotalRefunded,
                successful_payments: mergedSuccessful,
                failed_payments: mergedFailed,
                refunded_payments: mergedRefunded,
                success_rate: mergedTotalPayments
                    ? Math.round((mergedSuccessful / mergedTotalPayments) * 100)
                    : 0
            });
        }

        // Merge overall stats from individual + family payments
        const baseTotalPayments = parseInt(overallStats?.total_payments || 0);
        const baseTotalAmount = parseFloat(overallStats?.total_amount || 0);
        const baseSuccessful = parseInt(overallStats?.successful_payments || 0);
        const baseFailed = parseInt(overallStats?.failed_payments || 0);
        const baseRefunded = parseInt(overallStats?.refunded_payments || 0);
        const baseAverage = parseFloat(overallStats?.average_payment || 0);

        const baseSuccessAmount = baseAverage * baseSuccessful;

        const mergedTotalPayments = baseTotalPayments + familyPaymentCount;
        const mergedTotalAmount = baseTotalAmount + familyTotalPaid;
        const mergedSuccessful = baseSuccessful + familyPaymentCount;
        const mergedFailed = baseFailed;
        const mergedRefunded = baseRefunded;

        const mergedAverage =
            mergedSuccessful > 0
                ? (baseSuccessAmount + familyTotalPaid) / mergedSuccessful
                : 0;

        const mergedSuccessRate =
            mergedTotalPayments > 0
                ? Math.round((mergedSuccessful / mergedTotalPayments) * 100)
                : 0;
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                overall_stats: {
                    ...
                    failed_payments: mergedFailed,
                    refunded_payments: mergedRefunded,
                    average_payment: mergedAverage,
                    success_rate: mergedSuccessRate
                },
                latest_payment:
                    latestFamilyPayment &&
                    (!latestPayment ||
                        moment(latestFamilyPayment.created_at).isAfter(
                            moment(latestPayment.created_at)
                        ))
                        ? {
                              amount: parseFloat(latestFamilyPayment.amount),
                              currency: latestFamilyPayment.currency,
                              status: latestFamilyPayment.status,
                              date: latestFamilyPayment.created_at,
                              method: latestFamilyPayment.payment_method
                          }
                        : latestPayment
                        ? {
...
async function getRefundDetails(req, res) {
    try {
        ...
        const refundProcessorUser = transaction.RefundProcessor;
        const resolvedRefundProcessorName =
            transaction.refund_processed_by_name ||
            (refundProcessorUser && refundProcessorUser.full_name) ||
            null;
        ...
        const refundDetails = {
            ...
            processing: {
                ...
                processed_by_name: resolvedRefundProcessorName,
                processed_by_email: (refundProcessorUser && refundProcessorUser.email) || null
            },
            student: {
                id: student.id,
                name: student.full_name,
...
const addPrevUnusedLessons = async (req, res) => {
  ...
  try {
    ...
    const oldCurrentSubscriptionValue = currentSub.get({ plain: true });
    const oldPreviousSubscriptionValue = previousSub.get({ plain: true });
    ...
    await ActionLog.create(
      {
        actor_id: req.user.id,
        action_type: 'add previous unused lesson',
        target_entity: 'lesson',
        target_id: parseInt(id),
        old_value: {
          current_subscription: oldCurrentSubscriptionValue,
          previous_subscription: oldPreviousSubscriptionValue
        },
        new_value: {
          current_subscription: currentSub.get({ plain: true }),
          previous_subscription: previousSub.get({ plain: true }),
          moved_lessons: parseInt(prev_unused_lessons)
        },
        created_at: new Date()
      },
      { transaction: dbTransaction }
