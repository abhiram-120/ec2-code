...
const FamilyChild = sequelize.define(
    'FamilyChild',
    {
        ...
        pricing_type: {
            ...
            allowNull: true,
            comment:
                'default = standard tier pricing; custom = negotiated amount (from cart planType / payment)'
        },
        status: {
...
const FamilyCartItem = sequelize.define(
    'FamilyCartItem',
    {
        ...
        cart_pricing_type: {
            type: DataTypes.ENUM('default', 'custom'),
            allowNull: true,
            comment: 'Mirrors cart global plan type (Default vs Custom) for reload and reporting'
        },
        added_at: {
            type: DataTypes.DATE,
            defaultValue: DataTypes.NOW,
...
const FamilyPaymentTransaction = sequelize.define(
    'FamilyPaymentTransaction',
    {
        ...
        refund_amount: {
            type: DataTypes.DECIMAL(10, 2),
            allowNull: true,
            comment: 'Amount refunded for this transaction'
        },
        refund_type: {
            type: DataTypes.ENUM('full', 'partial'),
            allowNull: true,
            comment: 'Type of refund processed (full or partial)'
        },
        refund_reason: {
            type: DataTypes.STRING(255),
            allowNull: true,
            comment: 'Reason provided for the refund'
        },
        refund_date: {
            type: DataTypes.DATE,
            allowNull: true,
        ...
        refund_processed_by: {
            type: DataTypes.INTEGER.UNSIGNED,
            allowNull: true,
            comment: 'Admin user ID who processed the refund'
        },
        refund_processed_by_name: {
            type: DataTypes.STRING(255),
            allowNull: true,
            comment: 'Admin name who processed the refund'
        },
        email_notification_sent: {
            type: DataTypes.BOOLEAN,
            allowNull: true,
            defaultValue: false,
            comment: 'Whether refund email notification was sent to the family'
        },
        processed_at: {
            type: DataTypes.DATE,
            allowNull: true,
